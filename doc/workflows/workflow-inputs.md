...
검토 시 최신 요구사항 JSON도 같은 `tasks` 구조를 사용한다. 새 연동 업무에 새 runId를 사용하고 진행 중인 같은 목표·업무·요구사항의 후속 지시와 재시도는 기존 runId로 이어간다. 단순 중간 수정에는 start를 반복하지 않는다. 요구사항 변경이나 종료된 실행 이후의 새 작업은 이전 기록을 보존한 후 후속 실행으로 연결한다. 일반 작업에는 이 JSON을 만들지 않는다.
