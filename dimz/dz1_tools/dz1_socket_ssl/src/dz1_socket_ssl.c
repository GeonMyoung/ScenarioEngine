										  ...
										  Dz1SocketCallback2SSLVer ssl_ver, str_t certName, str_t keyName)
{
	...
	{	// Native Socket을 SSL Callback Mode로 바꾼다
		...
		Dz1SocketCallback2SpecSSL *ssl_user_ptr = NULL;
		Dz1SocketCallback2SpecSSL *ssl_spec = ssl_user_ptr = (Dz1SocketCallback2SpecSSL *)Dz1Calloc(sizeof(Dz1SocketCallback2SpecSSL), 1, errp);
		...
		else
		{
			...
			if (0) { }
																	...
																	&ssl_user_ptr, (Dz1DelFunc)Dz1SocketCallback2SpecSSL_del)).code) ERR_OUT(errp);
			else
			{
// 				sock->specific			= ssl_spec;
				...
				if (user_ptr != NULL)
				{
					ssl_spec->user_ptr = *user_ptr;
					if (ssl_spec->user_ptr_del != NULL)
						*user_ptr = NULL;
				}
...
Dz1Error Dz1SockUtilTcpClient_modeSSLServer(Dz1TcpClientSocket *sock,
											Dz1SockAddr *peer, Dz1SockAddr *local,
											u32_t buf_size,
//											Dz1TcpClientSocketSSLServerMode mode,
...
// 		if ((err = Dz1TcpClientSocket_modeSSLServer(sock, peer, local, buf_size,
// 												   Dz1SockUtil_callbackRx, (void *)rx_arg,
// 												   (Dz1DelFunc)Dz1SockUtilReceiverArg_del,
// 												   Dz1SockUtil_callbackTx, (void *)tx_arg,
// 												   (Dz1DelFunc)Dz1SockUtilReceiverArg_del,
// 												   mode, certName, keyName)).code) ERR_OUT(errp);
		if ((*errp = Dz1SocketCallback2SSL_assoServer(sock, peer, local, buf_size,
													  Dz1SockUtil_callbackRx,
													  Dz1SockUtil_callbackTx,
													  (void *)&cb_arg, (Dz1DelFunc)Dz1SockUtilReceiverArg_del,
													  mode, certName, keyName)).code) ERR_OUT(errp);
		else
		{
			cb_arg = NULL;
		...
		pthread_cleanup_pop(1); // (Dz1SockUtilReceiverArg_delAndSetNull, (void *)&tx_arg);
	}
	return err;
}

Dz1Error Dz1SockUtilTcpClient_modeSSLClient(Dz1TcpClientSocket *sock,
											Dz1SockAddr *peer, Dz1SockAddr *local,
											...
											str_t ciper_list, u32_t parentTaskQueueID)
{
	...
	{
		pthread_cleanup_push(Dz1SockUtilReceiverArg_delAndSetNull, (void *)&cb_arg);
// 		if ((err = Dz1TcpClientSocket_modeSSLClient(sock, peer, local, buf_size,
// 												   Dz1SockUtil_callbackRx, (void *)rx_arg,
// 												   (Dz1DelFunc)Dz1SockUtilReceiverArg_del,
// 												   Dz1SockUtil_callbackTx, (void *)tx_arg,
// 												   (Dz1DelFunc)Dz1SockUtilReceiverArg_del,
// 												   mode, ciper_list)).code) ERR_OUT(errp);
		if ((*errp = Dz1SocketCallback2SSL_assoClient(sock, peer, local, buf_size,
													  Dz1SockUtil_callbackRx,
													  Dz1SockUtil_callbackTx,
													  (void *)&cb_arg, (Dz1DelFunc)Dz1SockUtilReceiverArg_del,
													  mode, ciper_list)).code) ERR_OUT(errp);
...
Dz1Error Dz1SockUtilTcpClient_mode2SSLServer(u32_t sessionID,
...
// 		if ((err = Dz1TcpClientSocket_modeSSLServer(sock, peer, local, buf_size,
// 												   Dz1SockUtil_callback2Rx, (void *)rx_arg,
// 												   (Dz1DelFunc)Dz1SockUtilReceiver2Arg_del,
// 												   Dz1SockUtil_callback2Tx, (void *)tx_arg,
// 												   (Dz1DelFunc)Dz1SockUtilReceiver2Arg_del,
// 												   mode, certName, keyName)).code) ERR_OUT(errp);
		if ((*errp = Dz1SocketCallback2SSL_assoServer(sock, peer, local, buf_size,
													  Dz1SockUtil_callback2Rx,
												      Dz1SockUtil_callback2Tx,
													  (void *)&cb_arg, (Dz1DelFunc)Dz1SockUtilReceiver2Arg_del,
												      mode, certName, keyName)).code) ERR_OUT(errp);
		else
		{
			cb_arg = NULL;
		...
		pthread_cleanup_pop(1); // (Dz1SockUtilReceiver2Arg_delAndSetNull, (void *)&tx_arg);
	}
	return err;
}

Dz1Error Dz1SockUtilTcpClient_mode2SSLClient(u32_t sessionID,
											 Dz1TcpClientSocket *sock,
											 Dz1SockAddr *peer, Dz1SockAddr *local,
											 u32_t buf_size, // Dz1TcpClientSocketSSLClientMode mode,
											 Dz1SocketCallback2SSLVer mode,
