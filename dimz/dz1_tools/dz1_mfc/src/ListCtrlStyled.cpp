...
int CListCtrlStyled::InsertItem( UINT nMask, int nItem, LPCTSTR lpszItem, UINT nState, UINT nStateMask, int nImage, LPARAM lParam )
{
	...
	lvitem.iSubItem = 0;
#ifdef _UNICODE
	lvitem.pszText = (wchar_t *)lpszItem;
#else
	lvitem.pszText = (char *)lpszItem;
#endif
	lvitem.cchTextMax = 0;
#ifdef _UNICODE
	if(nMask & LVIF_TEXT) lvitem.cchTextMax = (int)wcslen(lvitem.pszText);
#else
	if(nMask & LVIF_TEXT) lvitem.cchTextMax = (int)strlen(lvitem.pszText);
#endif
	lvitem.state = nState;
...
LPARAM CListCtrlStyled::GetItemData( int nItem )		// Jake Lim
...
//	if(lpLS_item->mParam) return (DWORD)(lpLS_item->lParam);
	if(lpLS_item != NULL && lpLS_item->mParam) return (lpLS_item->lParam);
	else
		return NULL;	// No significant if the item mask hasn't LVIF_PARAM
}

//BOOL CListCtrlStyled::SetItemData( int nItem, DWORD dwData )
BOOL CListCtrlStyled::SetItemData( int nItem, LPARAM dwData )	// Jake Lim
{
	// Retrieve the LS_item structure
	//
	LVITEM pItem;
...
{	// Create an LVCOLUMN item
	...
	*/
	...
	lvColumn.cx = nWidth;
#ifdef _UNICODE
	lvColumn.pszText = (wchar_t *)lpszColumnHeading;
	lvColumn.cchTextMax = (int)wcslen( lvColumn.pszText );
#else
...
void CListCtrlStyled::OnCustomdraw(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    // below.
    int nItem = (int)(DWORD)pNMLVCUSTOMDRAW->nmcd.dwItemSpec;
...
#ifdef _UNICODE
	if (wcscmp(font->lfFaceName, this->m_Default_LOGFONT.lfFaceName) != 0)
		wcscpy(dest->lfFaceName, font->lfFaceName);
#else
