...
#ifdef WIN32
	if ((sh = Dz1Shell_new("win32crt", DZ1_SHELL_CMD_ALL, 16, "root", "dz1_tdc_info", &err)) == NULL) Dz1Error_print(&err, 0);
#else
	if ((sh = Dz1Shell_new("xterm", DZ1_SHELL_CMD_ALL, 16, "root", "dz1_tdc_info", &err)) == NULL) ERR_OUT(&err);
#endif
	else
	{
...
int main(int argc, char *argv[])
{
	...
	if ((logfp = fopen("dz1_tdc_infoTest.log.txt", "wb")) != NULL)
