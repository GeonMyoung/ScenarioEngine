...
8.23.4		Where a character string type is specified in X.680 by direct reference to an enumerating table (NumericString and PrintableString),
			...
			문자열 유형이 X.680에 지정된 열거 테이블(NumericString 및 PrintableString)을 직접 참조한 경우, 
			OCTET-STRING의 값은 8.23.5에 지정된 VisibleString 유형과 동일한 문자열을 갖는 값이어야 합니다.
...
8.23.5		For restricted character strings apart from UniversalString, UTF8String and BMPString, the octet string shall contain the octets specified 
			...
			UniversalString, UTF8String 및 BMPString을 제외한 제한된 문자열의 경우 OCTET-STRING은 ISO/IEC 2375에 따라 등록된 이스케이프 시퀀스 및 문자 코딩을 
			사용하여 8비트 환경의 인코딩을 위한 ISO/IEC 2022에 지정된 옥텟을 포함해야 합니다.
...
8.23.5.2	At the start of each string, certain registration numbers shall be assumed to be designated as G0 and/or C0 and/or C1, and invoked (using the terminology of ISO/IEC 2022).
			...
			These are specified for each type in Table 3, together with the assumed escape sequence they imply.
			이것들은 표 3의 각 유형에 대해 지정되며 이들이 암시하는 가정된 이스케이프 시퀀스와 함께 지정됩니다.
...
int _avoid_warning_no_symbol_restricted_str_ = 0;