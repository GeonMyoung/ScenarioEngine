...
static const u8_t aes_sbox[16][16] = {
...
static const u8_t aes_invsbox[16][16] = {
...
static const u8_t gf_mul[256][6] = {
...
static void xor_buf(const u8_t in[], u8_t out[], size_t len)
{
	size_t idx;
...
static u32_t SubWord(u32_t word)
{
	unsigned int result;
...
bool_t Dz1CryptAes_KeySetup(const u8_t key[], u32_t w[60], int keysize)
{
	int Nb = 4, Nr, Nk, idx;
	u32_t temp, Rcon[]=
	{
	...
	{
//		Dz1Thread_printf(Dz1Text("idx - 1 = %d, idx - Nk = %d\n"), idx - 1, idx - Nk);
		temp = w[idx - 1];
		if ((idx % Nk) == 0)
...
static void AddRoundKey(u8_t state[][4], const u32_t w[])
{
	u8_t subkey[4];
...
static void SubBytes(u8_t state[][4])
{
...
static void InvSubBytes(u8_t state[][4])
{
...
static void ShiftRows(u8_t state[][4])
{
	int t;
...
static void InvShiftRows(u8_t state[][4])
{
	int t;
...
static void MixColumns(u8_t state[][4])
{
	u8_t col[4];
...
static void InvMixColumns(u8_t state[][4])
{
	u8_t col[4];
...
// (En/De)Crypt
/////////////////
static void aes_encrypt(const u8_t in[], u8_t out[], const u32_t key[], int keysize)
{
	u8_t state[4][4];
...
static void aes_decrypt(const u8_t in[], u8_t out[], const u32_t key[], int keysize)
{
	u8_t state[4][4];
...
///////////////////////////////////////////////////////////////////////////////
// AES - CBC
int Dz1CryptAes_EncryptCBC(const u8_t in[], size_t in_len, u8_t out[], const u32_t key[60], int keysize, const u8_t iv[DZ1_CRYPT_AES_BLK_SIZE]) // modified by gm 20230602 - explicit the size of parameters
{
	u8_t buf_in[DZ1_CRYPT_AES_BLK_SIZE], buf_out[DZ1_CRYPT_AES_BLK_SIZE], iv_buf[DZ1_CRYPT_AES_BLK_SIZE];
	u32_t blocks, idx;

	if (in_len % DZ1_CRYPT_AES_BLK_SIZE != 0)
		return(FALSE);

	blocks = (u32_t)(in_len / DZ1_CRYPT_AES_BLK_SIZE);

	memcpy(iv_buf, iv, DZ1_CRYPT_AES_BLK_SIZE);

	for (idx = 0; idx < blocks; idx++) {
		memcpy(buf_in, &in[idx * DZ1_CRYPT_AES_BLK_SIZE], DZ1_CRYPT_AES_BLK_SIZE);
		xor_buf(iv_buf, buf_in, DZ1_CRYPT_AES_BLK_SIZE);
		aes_encrypt(buf_in, buf_out, key, keysize);
		memcpy(&out[idx * DZ1_CRYPT_AES_BLK_SIZE], buf_out, DZ1_CRYPT_AES_BLK_SIZE);
		memcpy(iv_buf, buf_out, DZ1_CRYPT_AES_BLK_SIZE);
	}

	return(TRUE);
}

int Dz1CryptAes_EncryptCBC_MAC(const u8_t in[], size_t in_len, u8_t out[], const u32_t key[60], int keysize, const u8_t iv[DZ1_CRYPT_AES_BLK_SIZE]) // modified by gm 20230602 - explicit the size of parameters
{
	u8_t buf_in[DZ1_CRYPT_AES_BLK_SIZE], buf_out[DZ1_CRYPT_AES_BLK_SIZE], iv_buf[DZ1_CRYPT_AES_BLK_SIZE];
	u32_t blocks, idx;
	...
	memcpy(out, buf_out, DZ1_CRYPT_AES_BLK_SIZE);   // Only output the last block.

	return(TRUE);
}

int Dz1CryptAes_DecryptCBC(const u8_t in[], size_t in_len, u8_t out[], const u32_t key[60], int keysize, const u8_t iv[DZ1_CRYPT_AES_BLK_SIZE]) // modified by gm 20230602 - explicit the size of parameters
{
	u8_t buf_in[DZ1_CRYPT_AES_BLK_SIZE], buf_out[DZ1_CRYPT_AES_BLK_SIZE], iv_buf[DZ1_CRYPT_AES_BLK_SIZE];
	u32_t blocks, idx;
	...
	for (idx = 0; idx < blocks; idx++) {
		...
		xor_buf(iv_buf, buf_out, DZ1_CRYPT_AES_BLK_SIZE);
		memcpy(&out[idx * DZ1_CRYPT_AES_BLK_SIZE], buf_out, DZ1_CRYPT_AES_BLK_SIZE);
		memcpy(iv_buf, buf_in, DZ1_CRYPT_AES_BLK_SIZE);
	}
...
* AES - CTR
*******************/
static void increment_iv(u8_t iv[], int counter_size)
{
	int idx;

	// Use counter_size bytes at the end of the IV as the big-endian integer to increment.
	for (idx = DZ1_CRYPT_AES_BLK_SIZE - 1; idx >= DZ1_CRYPT_AES_BLK_SIZE - counter_size; idx--) {
		iv[idx]++;
		if (iv[idx] != 0 || idx == DZ1_CRYPT_AES_BLK_SIZE - counter_size)
			break;
	}
}
...
void Dz1CryptAes_EncryptCTR(const u8_t in[], size_t in_len, u8_t out[], const u32_t key[60], int keysize, const u8_t iv[DZ1_CRYPT_AES_BLK_SIZE]) // modified by gm 20230602 - explicit the size of parameters
{
	size_t idx = 0, last_block_length;
	u8_t iv_buf[DZ1_CRYPT_AES_BLK_SIZE], out_buf[DZ1_CRYPT_AES_BLK_SIZE];

	if (in != out)
		memcpy(out, in, in_len);

	memcpy(iv_buf, iv, DZ1_CRYPT_AES_BLK_SIZE);
	last_block_length = in_len - DZ1_CRYPT_AES_BLK_SIZE;

	if (in_len > DZ1_CRYPT_AES_BLK_SIZE) {
		for (idx = 0; idx < last_block_length; idx += DZ1_CRYPT_AES_BLK_SIZE) {
			aes_encrypt(iv_buf, out_buf, key, keysize);
			xor_buf(out_buf, &out[idx], DZ1_CRYPT_AES_BLK_SIZE);
			increment_iv(iv_buf, DZ1_CRYPT_AES_BLK_SIZE);
		}
	}
...
void Dz1CryptAes_DecryptCTR(const u8_t in[], size_t in_len, u8_t out[], const u32_t key[60], int keysize, const u8_t iv[DZ1_CRYPT_AES_BLK_SIZE]) // modified by gm 20230602 - explicit the size of parameters
{
	// CTR encryption is its own inverse function.
	Dz1CryptAes_EncryptCTR(in, in_len, out, key, keysize, iv);
}
...
static void ccm_prepare_first_ctr_blk(u8_t counter[], const u8_t nonce[], int nonce_len, int payload_len_store_size)
{
	memset(counter, 0, DZ1_CRYPT_AES_BLK_SIZE);
	counter[0] = (payload_len_store_size - 1) & 0x07;
	memcpy(&counter[1], nonce, nonce_len);
}

static void ccm_prepare_first_format_blk(u8_t buf[], int assoc_len, int payload_len, int payload_len_store_size, int mac_len, const u8_t nonce[], int nonce_len)
{
	...
	memset(&buf[1 + nonce_len], 0, DZ1_CRYPT_AES_BLK_SIZE - 1 - nonce_len);
	buf[15] = payload_len & 0x000000FF;
	buf[14] = (payload_len >> 8) & 0x000000FF;
}

static void ccm_format_assoc_data(u8_t buf[], int *end_of_buf, const u8_t assoc[], int assoc_len)
{
	int pad;
	...
	*end_of_buf += assoc_len;
	pad = DZ1_CRYPT_AES_BLK_SIZE - (*end_of_buf % DZ1_CRYPT_AES_BLK_SIZE); /*BUG?*/
	memset(&buf[*end_of_buf], 0, pad);
	*end_of_buf += pad;
}

static void ccm_format_payload_data(u8_t buf[], int *end_of_buf, const u8_t payload[], int payload_len)
{
	int pad;

	memcpy(&buf[*end_of_buf], payload, payload_len);
	*end_of_buf += payload_len;
	pad = *end_of_buf % DZ1_CRYPT_AES_BLK_SIZE;
	if (pad != 0)
		pad = DZ1_CRYPT_AES_BLK_SIZE - pad;
	memset(&buf[*end_of_buf], 0, pad);
	*end_of_buf += pad;
}

// out_len = payload_len + assoc_len
int Dz1CryptAes_EncryptCCM(const u8_t payload[], u32_t payload_len, 
						   const u8_t assoc[], u16_t assoc_len,
						   const u8_t nonce[], u16_t nonce_len, 
						   u8_t out[], u32_t *out_len, u32_t mac_len, 
						   const u8_t key_str[], int keysize)
{
	u8_t temp_iv[DZ1_CRYPT_AES_BLK_SIZE], counter[DZ1_CRYPT_AES_BLK_SIZE], mac[16], *buf;
	...
	buf = (u8_t*)malloc(payload_len + assoc_len + 48 /*Round both payload and associated data up a block size and add an extra block.*/);
	if (! buf)
		return(FALSE);

	// Prepare the key for usage.
	Dz1CryptAes_KeySetup(key_str, key, keysize);

	// Format the first block of the formatted data.
	payload_len_store_size = DZ1_CRYPT_AES_BLK_SIZE - 1 - nonce_len;
	ccm_prepare_first_format_blk(buf, assoc_len, payload_len, payload_len_store_size, mac_len, nonce, nonce_len);
	end_of_buf = DZ1_CRYPT_AES_BLK_SIZE;
	...
	memset(temp_iv, 0, DZ1_CRYPT_AES_BLK_SIZE);
	Dz1CryptAes_EncryptCBC_MAC(buf, end_of_buf, mac, key, keysize, temp_iv);
	...
	memcpy(temp_iv, counter, DZ1_CRYPT_AES_BLK_SIZE);
	increment_iv(temp_iv, DZ1_CRYPT_AES_BLK_SIZE - 1 - mac_len);   // Last argument is the byte size of the counting portion of the counter block. /*BUG?*/
	Dz1CryptAes_EncryptCTR(out, payload_len, out, key, keysize, temp_iv);

	// Encrypt the MAC with CTR mode with a counter starting at 0.
	Dz1CryptAes_EncryptCTR(&out[payload_len], mac_len, &out[payload_len], key, keysize, counter);
...
int Dz1CryptAes_DecryptCCM(const u8_t ciphertext[], u32_t ciphertext_len, 
						   const u8_t assoc[], u16_t assoc_len,
						   const u8_t nonce[], u16_t nonce_len, 
						   u8_t plaintext[], u32_t *plaintext_len, u32_t mac_len, int *mac_auth, 
						   const u8_t key_str[], int keysize)
{
	u8_t temp_iv[DZ1_CRYPT_AES_BLK_SIZE], counter[DZ1_CRYPT_AES_BLK_SIZE], mac[16], mac_buf[16], *buf;
	...
	buf = (u8_t*)malloc(assoc_len + ciphertext_len /*ciphertext_len = plaintext_len + mac_len*/ + 48);
	if (! buf)
		return(FALSE);
	...
	*plaintext_len = ciphertext_len - mac_len;
	plaintext_len_store_size = DZ1_CRYPT_AES_BLK_SIZE - 1 - nonce_len;
	...
	memcpy(temp_iv, counter, DZ1_CRYPT_AES_BLK_SIZE);
	increment_iv(temp_iv, DZ1_CRYPT_AES_BLK_SIZE - 1 - mac_len);   // (DZ1_CRYPT_AES_BLK_SIZE - 1 - mac_len) is the byte size of the counting portion of the counter block.
	Dz1CryptAes_DecryptCTR(plaintext, *plaintext_len, plaintext, key, keysize, temp_iv);

	// Setting mac_auth to NULL disables the authentication check.
	if (mac_auth != NULL) {
		// Decrypt the MAC with CTR mode with a counter starting at 0.
		Dz1CryptAes_DecryptCTR(mac, mac_len, mac, key, keysize, counter);
		...
		// Perform the CBC operation with an IV of zeros on the formatted buffer to calculate the MAC.
		memset(temp_iv, 0, DZ1_CRYPT_AES_BLK_SIZE);
		Dz1CryptAes_EncryptCBC_MAC(buf, end_of_buf, mac_buf, key, keysize, temp_iv);
