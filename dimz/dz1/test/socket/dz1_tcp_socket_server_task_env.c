...
void ReceiverEntry_del(ReceiverEntry *p)
{
	...
	if (p->tx) Dz1SocketFifo_del(p->tx);
	Dz1Free(p);
}
...
void ReceiverEntry_dump(ReceiverEntry *p, int tab)
{
	...
	if (p->tx == NULL) Dz1Thread_tprintf(tab, "tx = NULL\n");
	else { Dz1Thread_tprintf(tab, "tx = %p\n", p->tx); }
...
Dz1TcpSocketServerTaskEnv *Dz1TcpSocketServerTaskEnv_new(
			void *tSelf, Dz1Error *err)
{
	Dz1Error _err = DZ1_ERROR_INITIALIZER, *errp = err ? err : &_err;
	Dz1TcpSocketServerTaskEnv *__internal_ret = (Dz1TcpSocketServerTaskEnv *)Dz1Calloc(sizeof(Dz1TcpSocketServerTaskEnv), 1, errp);
	if (__internal_ret == NULL) { ERR_SET_OUT(errp, ENOMEM); }
	else
	{
		pthread_cleanup_push(Dz1TcpSocketServerTaskEnv_delAndSetNull, (void *)&__internal_ret);

		if ((__internal_ret->acceptors = AcceptorList_new(errp)) == NULL) ERR_OUT(errp);
		else if ((__internal_ret->receivers = ReceiverList_new(errp)) == NULL) ERR_OUT(errp);
		else
		{
			__internal_ret->tSelf = tSelf;
			ERR_CLEAR(errp);
		}
		pthread_cleanup_pop(ERR_PROBE(errp)); // (Dz1TcpSocketServerTaskEnv_delAndSetNull, (void *)&__internal_ret)
	}
	return __internal_ret;
}

void Dz1TcpSocketServerTaskEnv_del(Dz1TcpSocketServerTaskEnv *p)
{
	if (p == NULL) return;
	if (p->acceptors) AcceptorList_del(p->acceptors);
	if (p->receivers) ReceiverList_del(p->receivers);
	Dz1Free(p);
}

void Dz1TcpSocketServerTaskEnv_dump(Dz1TcpSocketServerTaskEnv *p, int tab)
{
	if (!p) { Dz1Thread_printf("NULL\n"); return; }
	Dz1Thread_printf("{\n"); tab++;
	...
	if (p->acceptors == NULL) Dz1Thread_tprintf(tab, "acceptors = NULL\n");
	else { Dz1Thread_tprintf(tab, "acceptors = "); AcceptorList_dump(p->acceptors, tab); }

	if (p->receivers == NULL) Dz1Thread_tprintf(tab, "receivers = NULL\n");
	else { Dz1Thread_tprintf(tab, "receivers = "); ReceiverList_dump(p->receivers, tab); }
