...
////////////////////////////////////////////////////////////////////////////////
// cmd_test1 Root Command
static void cmd_test1_usage(Dz1Str n) { Dz1Thread_printf(Dz1Text("Usage] %s .\n"), n); }
static Dz1ShellCmdReturn cmd_test1(void* ptr, int argc, Dz1Str argv[])
{
	DZ1_ERROR_SAFE_VAR(errp, err);
	void (*_print_usage)(Dz1Str n) = cmd_test1_usage;
	Dz1Str n = argv[0];
	TestEnv *env = (TestEnv *)ptr;
	...
	{
		// Command Code Here
		if (Iso14827TestHeleprTest_test(env->task, errp) == FALSE) ERR_OUT(errp);
...
////////////////////////////////////////////////////////////////////////////////
// cmd_test1 Root Command
static void cmd_tr1_usage(Dz1Str n) { Dz1Thread_printf(Dz1Text("Usage] %s .\n"), n); }
static Dz1ShellCmdReturn cmd_tr1(void* ptr, int argc, Dz1Str argv[])
{
	DZ1_ERROR_SAFE_VAR(errp, err);
	void (*_print_usage)(Dz1Str n) = cmd_tr1_usage;
	Dz1Str n = argv[0];
	TestEnv* env = (TestEnv*)ptr;
	if (argc != 1) _print_usage(n);
	else
	{
		// Command Code Here
		if (Iso14827TestHeleprTest_tr1(env->task, errp) == FALSE) ERR_OUT(errp);
		else Dz1Error_set(errp, 0);
	}
	return Dz1ShellCmdReturn_ok;
}
// cmd_tr1 Root Command
////////////////////////////////////////////////////////////////////////////////
...
#ifdef WIN32
	if ((sh = Dz1Shell_new(Dz1Text("win32crt"), DZ1_SHELL_CMD_ALL, 16, Dz1Text("root"), Dz1Text("Iso14827TestHelper"), errp)) == NULL) ERR_OUT(errp);
#else
	if ((sh = Dz1Shell_new(Dz1Text("xterm"), DZ1_SHELL_CMD_ALL, 16, Dz1Text("root"), Dz1Text("Iso14827TestHelper"), errp)) == NULL) ERR_OUT(errp);
#endif
	else
	{
		...
		else if ((*errp = Dz1Shell_cmdReg(sh, Dz1Text("test1"), cmd_test1, task, Dz1Text("datext data packet sample decode"))).code) ERR_OUT(errp);
		else if ((*errp = Dz1Shell_cmdReg(sh, Dz1Text("tr1"), cmd_tr1, task, Dz1Text("tr1 test") )).code) ERR_OUT(errp);
		else
		{
...
{
	Dz1Thread_log(DZ1_THREAD_LOG_ERROR, "Iso14827TestHelper : Exception Catch\n");
	ERR_OUT(reason);
}
...
static bool_t _process_start(bool_t is_daemon, Dz1Error *errp)
{
	...
	if ((logfp = Dz1FileStream_open(Dz1Text("Iso14827TestHelperTest.log.txt"), Dz1Text("wb"), NULL)) != NULL)
	{
	...
	else
	{
		...
		if ((env.task = Iso14827TestHelper_new(NULL, 0, _task_exception_handler, NULL, errp)) == NULL) ERR_OUT(errp);	// Edit Task Argument
