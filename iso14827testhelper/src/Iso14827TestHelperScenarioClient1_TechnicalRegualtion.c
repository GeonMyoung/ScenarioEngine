...
// technical regulation 1
static Dz1Str test_name_test_pub_current_link_state = Dz1T("test_pub_current_link_state");
static Dz1Str test_name_test_pub_event_identity = Dz1T("test_pub_event_identity");
static Dz1Str test_name_test_pub_incident_identity = Dz1T("test_pub_incident_identity");
static Dz1Str test_name_test_pub_incident_conditions = Dz1T("test_pub_incident_conditions");
static Dz1Str test_name_test_pub_roadway_surface_status = Dz1T("test_pub_roadway_surface_status");
static Dz1Str test_name_test_pub_weather_information = Dz1T("test_pub_weather_information");
static Dz1Str test_name_test_pub_link_roadway_geometry = Dz1T("test_pub_link_roadway_geometry");
static Dz1Str test_name_test_pub_probe_vehicle_detection = Dz1T("test_pub_probe_vehicle_detection");
static Dz1Str test_name_test_pub_detector_collection = Dz1T("test_pub_detector_collection");
static Dz1Str test_name_test_pub_extention_comm_pdu = Dz1T("test_pub_extention_comm_pdu");

// technical regulation2
static Dz1Str test_name_test_pub_comm_tx_pdu = Dz1T("test_pub_comm_tx_pdu");
static Dz1Str test_name_test_pub_comm_rx_pdu = Dz1T("test_pub_comm_rx_pdu");
...
static bool_t _client_establish_session(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, Dz1SockAddr* peer, Dz1Asn1UTF8Str* peer_domain, Iso14827TestClientAccount* acc, Dz1Error* errp)
{
	...
	Iso14827TestClientRole1TR* s = env->scenario->x.client1_tr;
	if (s->test_session)
	{
...
///////////////////////////////////////////////////////////////////////////////
// setup passive mode
static bool_t _client_passive_prepare(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, Iso14827TestClientPassive* c, Dz1Error* errp)
{
...
static bool_t _client_connect_and_test_login(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, Iso14827TestClientActive* c, Dz1Error* errp)
{
	u32_t my_queue_id = Dz1Task_getQueueID(env->tSelf);
...
static bool_t _client1_echo_responsed(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, ItsIso14827EventDisconnected* msg, Dz1Error* errp)
{
...
///////////////////////////////////////////////////////////////////////////////
// INITIAL State
static bool_t _client1_initial_start(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, Dz1Error* errp)
{
	// start event handler
...
{
	Iso14827TestClientRole1TR* s = (Iso14827TestClientRole1TR*)_scenario;
...
static bool_t _client1_login_duplicate_test(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, Dz1Error* errp)
{
...
static bool_t _client1_expected_disconnected_received(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, ItsIso14827EventDisconnected* msg, Dz1Error* errp)
{
	Iso14827TestClientAccount* acc = env->primary_acc;
...
static bool_t _client1_invalid_subc_test(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, Dz1Error* errp)
{
...
static bool_t _client1_data_control_received(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, ItsIso14827ClientEventPubControl* msg, Dz1Error* errp)
{
	SubscribeSpecEntry* entry = NULL;
	SubscribeSpecList* list = NULL;
...
static bool_t _start_normal_pub_by_id(Iso14827TestHelperEnv* env, Iso14827TestClientRole1TR* s, Dz1Str testScenario, Dz1Str oidStr, Dz1Error* errp)
{
	if (testScenario == NULL || oidStr == NULL) ERR_SET_OUT(errp, EFAULT);
	else
	{
		if (Dz1STRCMP(testScenario, Dz1T("TechnicalRegulation1-clientRole1")) == 0)
		{
			if (Dz1STRCMP(oidStr, Dz1T("1.0.14827.1.0.1")) == 0 && s->validate_set->x.technical_regulation1_clientRole1->test_pub_current_link_state)
			{
				if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_current_link_state, Dz1T("START"), errp) == FALSE) ERR_OUT(errp);
				else Dz1Error_set(errp, 0);
			}
			else if (Dz1STRCMP(oidStr, Dz1T("1.0.14827.1.0.2")) == 0 && s->validate_set->x.technical_regulation1_clientRole1->test_pub_event_identity)
			{
				if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_event_identity, Dz1T("START"), errp) == FALSE) ERR_OUT(errp);
				else Dz1Error_set(errp, 0);
			}
			else if (Dz1STRCMP(oidStr, Dz1T("1.0.14827.1.0.3")) == 0 && s->validate_set->x.technical_regulation1_clientRole1->test_pub_incident_identity)
			{
				if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_incident_identity, Dz1T("START"), errp) == FALSE) ERR_OUT(errp);
				else Dz1Error_set(errp, 0);
			}
			else if (Dz1STRCMP(oidStr, Dz1T("1.0.14827.1.0.4")) == 0 && s->validate_set->x.technical_regulation1_clientRole1->test_pub_incident_conditions)
			{
				if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_incident_conditions, Dz1T("START"), errp) == FALSE) ERR_OUT(errp);
				else Dz1Error_set(errp, 0);
			}
			else if (Dz1STRCMP(oidStr, Dz1T("1.0.14827.1.0.5")) == 0 && s->validate_set->x.technical_regulation1_clientRole1->test_pub_roadway_surface_status)
			{
				if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_roadway_surface_status, Dz1T("START"), errp) == FALSE) ERR_OUT(errp);
				else Dz1Error_set(errp, 0);
			}
			else if (Dz1STRCMP(oidStr, Dz1T("1.0.14827.1.0.6")) == 0 && s->validate_set->x.technical_regulation1_clientRole1->test_pub_weather_information)
			{
				if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_weather_information, Dz1T("START"), errp) == FALSE) ERR_OUT(errp);
				else Dz1Error_set(errp, 0);
			}
			else if (Dz1STRCMP(oidStr, Dz1T("1.0.14827.1.0.7")) == 0 && s->validate_set->x.technical_regulation1_clientRole1->test_pub_link_roadway_geometry)
			{
				if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_link_roadway_geometry, Dz1T("START"), errp) == FALSE) ERR_OUT(errp);
				else Dz1Error_set(errp, 0);
			}
			else if (Dz1STRCMP(oidStr, Dz1T("1.0.14827.1.0.8")) == 0 && s->validate_set->x.technical_regulation1_clientRole1->test_pub_probe_vehicle_detection)
			{
				if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_probe_vehicle_detection, Dz1T("START"), errp) == FALSE) ERR_OUT(errp);
				else Dz1Error_set(errp, 0);
			}
			else if (Dz1STRCMP(oidStr, Dz1T("1.0.14827.1.0.9")) == 0 && s->validate_set->x.technical_regulation1_clientRole1->test_pub_detector_collection)
			{
				if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_detector_collection, Dz1T("START"), errp) == FALSE) ERR_OUT(errp);
				else Dz1Error_set(errp, 0);
			}
			else if (Dz1STRCMP(oidStr, Dz1T("1.0.14827.1.1.0")) == 0 && s->validate_set->x.technical_regulation1_clientRole1->test_pub_extention_comm_pdu)
			{
				if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_extention_comm_pdu, Dz1T("START"), errp) == FALSE) ERR_OUT(errp);
				else Dz1Error_set(errp, 0);
			}
			else ERR_SET_OUT(errp, EPERM);
		}
		else if (Dz1STRCMP(testScenario, Dz1T("TechnicalRegulation2-clientRole1")) == 0)
		{
			if (Dz1STRCMP(oidStr, Dz1T("1.0.15784.2.0.1")) == 0 && s->validate_set->x.technical_regulation2_clientRole1->test_pub_comm_tx_pdu)
			{
				if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_comm_tx_pdu, Dz1T("START"), errp) == FALSE) ERR_OUT(errp);
				else Dz1Error_set(errp, 0);
			}
			else if (Dz1STRCMP(oidStr, Dz1T("1.0.15784.2.0.2")) == 0 && s->validate_set->x.technical_regulation2_clientRole1->test_pub_comm_rx_pdu)
			{
				if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_comm_rx_pdu, Dz1T("START"), errp) == FALSE) ERR_OUT(errp);
				else Dz1Error_set(errp, 0);
			}
			else ERR_SET_OUT(errp, EPERM);
		}
...
///////////////////////////////////////////////////////////////////////////////
// do normal subscribe
static bool_t _client1_normal_subc_test(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, Dz1Error* errp)
{
	Iso14827RejectSubscription result;
	SubscribeSpecEntry* entry = NULL;
	SubscribeSpecList* list = NULL;
	...
	{
		if ((env->flags & CLIENT_FLAG_SUBS_NORMAL_COMPLETE) == 0)
		{
			// 정보제공시험할때만
			if (_start_normal_pub_by_id(env, s, arg->testScenario, entry->oid, errp) == FALSE) ERR_OUT(errp);
		}

		if (_client_subscribe(arg, env, s->peer_domain, acc, entry, FALSE, &env->test_subs_id, &result, errp) == FALSE) ERR_OUT(errp);
...
static bool_t _received_validate_rsp(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, void* _msg, Dz1Error* errp)
{
...
static bool_t _client1_normal_subs_data_received(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, ItsIso14827ClientEventPublication* msg, Dz1Error* errp)
{
	...
	else
	{
		...
		if (api != NULL)
		{
			...
			Dz1Binary* data = NULL;
			// 기본교통정보교환 1
			if (Dz1STRCMP(arg->testScenario, Dz1T("TechnicalRegulation1-clientRole1")) == 0)
			{
				if (Dz1STRCMP(Dz1T("1.0.14827.1.0.1"), msg->oidStr) == 0)
				{
					Dz1CurrentLinkStateResponse* rsp = NULL;
					if ((rsp = (Dz1CurrentLinkStateResponse*)api->struct_decode(&bin, arg->initial_codec, errp)) == NULL) ERR_OUT(errp);
					else
					{
						CurrentLinkStateList* list = NULL;
						pthread_cleanup_push(Dz1CurrentLinkStateResponse_delAndSetNull, (void*)&rsp);
						if ((list = CurrentLinkStateList_fromDimz(rsp, errp)) == NULL) ERR_OUT(errp);
						else
						{
							pthread_cleanup_push(CurrentLinkStateList_delAndSetNull, (void*)&list);
							if ((str_u8 = _get_stringify_u8(env->spec, Dz1T("CurrentLinkStateList"), list, errp)) == NULL) ERR_OUT(errp);
							else
							{
								pthread_cleanup_push(Dz1Asn1UTF8Str_delAndSetNull, (void*)&str_u8);
								if ((data = Dz1Binary_new(str_u8->data, str_u8->size, errp)) == NULL) ERR_OUT(errp);
								...
								{
									pthread_cleanup_push(Dz1Binary_delAndSetNull, (void*)&data);
									if (Iso14827TestHelperEnvUtil_notifyInfo(env, test_name_test_pub_current_link_state, NULL, Dz1T("교통소통정보 정보 제공을 확인하였습니다."), data, Dz1T("JSON"), errp) == FALSE) ERR_OUT(errp);
									else Dz1Error_set(errp, 0);

									if (s->validate_set->x.technical_regulation1_clientRole1->test_pub_current_link_state)
									{
										if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_current_link_state, Dz1T("SUCCESS"), errp) == FALSE) ERR_OUT(errp);
										else Dz1Error_set(errp, 0);
									}
									pthread_cleanup_pop(1); // (Dz1Binary_delAndSetNull, (void*)&data);
								}
							...
							pthread_cleanup_pop(1); // (CurrentLinkStateList_delAndSetNull, (void*)&list);
						}
						pthread_cleanup_pop(1); // (Dz1CurrentLinkStateResponse_delAndSetNull, (void*)&rsp);
					}
				}
				else if (Dz1STRCMP(Dz1T("1.0.14827.1.0.2"), msg->oidStr) == 0)
				{
					Dz1EventIdentityResponse* rsp = NULL;
					if ((rsp = (Dz1EventIdentityResponse*)api->struct_decode(&bin, arg->initial_codec, errp)) == NULL) ERR_OUT(errp);
					else
					{
						Dz1EventIdentityResponse_dump(rsp, 4);
						EventIdentityList* list = NULL;
						pthread_cleanup_push(Dz1EventIdentityResponse_delAndSetNull, (void*)&rsp);
						if ((list = EventIdentityList_fromDimz(rsp, errp)) == NULL) ERR_OUT(errp);
						else
						{
							EventIdentityList_dump(list, 4);

							pthread_cleanup_push(EventIdentityList_delAndSetNull, (void*)&list);
							if ((str_u8 = _get_stringify_u8(env->spec, Dz1T("EventIdentityList"), list, errp)) == NULL) ERR_OUT(errp);
							else
							{
								pthread_cleanup_push(Dz1Asn1UTF8Str_delAndSetNull, (void*)&str_u8);
								if ((data = Dz1Binary_new(str_u8->data, str_u8->size, errp)) == NULL) ERR_OUT(errp);
								...
								{
									pthread_cleanup_push(Dz1Binary_delAndSetNull, (void*)&data);
									if (Iso14827TestHelperEnvUtil_notifyInfo(env, test_name_test_pub_current_link_state, NULL, Dz1T("교통통제정보 정보 제공을 확인하였습니다."), data, Dz1T("JSON"), errp) == FALSE) ERR_OUT(errp);
									else Dz1Error_set(errp, 0);

									if (s->validate_set->x.technical_regulation1_clientRole1->test_pub_event_identity)
									{
										if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_event_identity, Dz1T("SUCCESS"), errp) == FALSE) ERR_OUT(errp);
										else Dz1Error_set(errp, 0);
									}
									pthread_cleanup_pop(1); // (Dz1Binary_delAndSetNull, (void*)&data);
								}
							...
							pthread_cleanup_pop(1); // (EventIdentityList_delAndSetNull, (void*)&list);
						}
						pthread_cleanup_pop(1); // (Dz1EventIdentityResponse_delAndSetNull, (void*)&rsp);
					}
				}
				else if (Dz1STRCMP(Dz1T("1.0.14827.1.0.3"), msg->oidStr) == 0)
				{
					Dz1IncidentIdentityResponse* rsp = NULL;
					if ((rsp = (Dz1IncidentIdentityResponse*)api->struct_decode(&bin, arg->initial_codec, errp)) == NULL) ERR_OUT(errp);
					else
					{
						IncidentIdentityList* list = NULL;
						pthread_cleanup_push(Dz1IncidentIdentityResponse_delAndSetNull, (void*)&rsp);
						if ((list = IncidentIdentityList_fromDimz(rsp, errp)) == NULL) ERR_OUT(errp);
						else
						{
							pthread_cleanup_push(IncidentIdentityList_delAndSetNull, (void*)&list);
							if ((str_u8 = _get_stringify_u8(env->spec, Dz1T("IncidentIdentityList"), list, errp)) == NULL) ERR_OUT(errp);
							else
							{
								pthread_cleanup_push(Dz1Asn1UTF8Str_delAndSetNull, (void*)&str_u8);
								if ((data = Dz1Binary_new(str_u8->data, str_u8->size, errp)) == NULL) ERR_OUT(errp);
								...
								{
									pthread_cleanup_push(Dz1Binary_delAndSetNull, (void*)&data);
									if (Iso14827TestHelperEnvUtil_notifyInfo(env, test_name_test_pub_current_link_state, NULL, Dz1T("돌발상황 발생정보 정보 제공을 확인하였습니다."), data, Dz1T("JSON"), errp) == FALSE) ERR_OUT(errp);
									else Dz1Error_set(errp, 0);

									if (s->validate_set->x.technical_regulation1_clientRole1->test_pub_incident_identity)
									{
										if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_incident_identity, Dz1T("SUCCESS"), errp) == FALSE) ERR_OUT(errp);
										else Dz1Error_set(errp, 0);
									}
									pthread_cleanup_pop(1); // (Dz1Binary_delAndSetNull, (void*)&data);
								}
							...
							pthread_cleanup_pop(1); // (IncidentIdentityList_delAndSetNull, (void*)&list);
						}
						pthread_cleanup_pop(1); // (Dz1IncidentIdentityResponse_delAndSetNull, (void*)&rsp);
					}
				}
				else if (Dz1STRCMP(Dz1T("1.0.14827.1.0.4"), msg->oidStr) == 0)
				{
					Dz1IncidentConditionsResponse* rsp = NULL;
					if ((rsp = (Dz1IncidentConditionsResponse*)api->struct_decode(&bin, arg->initial_codec, errp)) == NULL) ERR_OUT(errp);
					else
					{
						IncidentConditionsList* list = NULL;
						pthread_cleanup_push(Dz1IncidentConditionsResponse_delAndSetNull, (void*)&rsp);
						if ((list = IncidentConditionsList_fromDimz(rsp, errp)) == NULL) ERR_OUT(errp);
						else
						{
							pthread_cleanup_push(IncidentConditionsList_delAndSetNull, (void*)&list);
							if ((str_u8 = _get_stringify_u8(env->spec, Dz1T("IncidentConditionsList"), list, errp)) == NULL) ERR_OUT(errp);
							else
							{
								pthread_cleanup_push(Dz1Asn1UTF8Str_delAndSetNull, (void*)&str_u8);
								if ((data = Dz1Binary_new(str_u8->data, str_u8->size, errp)) == NULL) ERR_OUT(errp);
								...
								{
									pthread_cleanup_push(Dz1Binary_delAndSetNull, (void*)&data);
									if (Iso14827TestHelperEnvUtil_notifyInfo(env, test_name_test_pub_current_link_state, NULL, Dz1T("돌발상황정보 정보 제공을 확인하였습니다."), data, Dz1T("JSON"), errp) == FALSE) ERR_OUT(errp);
									else Dz1Error_set(errp, 0);

									if (s->validate_set->x.technical_regulation1_clientRole1->test_pub_incident_conditions)
									{
										if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_incident_conditions, Dz1T("SUCCESS"), errp) == FALSE) ERR_OUT(errp);
										else Dz1Error_set(errp, 0);
									}
									pthread_cleanup_pop(1); // (Dz1Binary_delAndSetNull, (void*)&data);
								}
							...
							pthread_cleanup_pop(1); // (IncidentConditionsList_delAndSetNull, (void*)&list);
						}
						pthread_cleanup_pop(1); // (Dz1IncidentConditionsResponse_delAndSetNull, (void*)&rsp);
					}
				}
				else if (Dz1STRCMP(Dz1T("1.0.14827.1.0.5"), msg->oidStr) == 0)
				{
					Dz1RoadwaySurfaceStatusResponse* rsp = NULL;
					if ((rsp = (Dz1RoadwaySurfaceStatusResponse*)api->struct_decode(&bin, arg->initial_codec, errp)) == NULL) ERR_OUT(errp);
					else
					{

						RoadwaySurfaceStatusList* list = NULL;
						pthread_cleanup_push(Dz1RoadwaySurfaceStatusResponse_delAndSetNull, (void*)&rsp);
						if ((list = RoadwaySurfaceStatusList_fromDimz(rsp, errp)) == NULL) ERR_OUT(errp);
						else
						{
							pthread_cleanup_push(RoadwaySurfaceStatusList_delAndSetNull, (void*)&list);
							if ((str_u8 = _get_stringify_u8(env->spec, Dz1T("RoadwaySurfaceStatusList"), list, errp)) == NULL) ERR_OUT(errp);
							else
							{
								pthread_cleanup_push(Dz1Asn1UTF8Str_delAndSetNull, (void*)&str_u8);
								if ((data = Dz1Binary_new(str_u8->data, str_u8->size, errp)) == NULL) ERR_OUT(errp);
								...
								{
									pthread_cleanup_push(Dz1Binary_delAndSetNull, (void*)&data);
									if (Iso14827TestHelperEnvUtil_notifyInfo(env, test_name_test_pub_current_link_state, NULL, Dz1T("도로상태정보 정보 제공을 확인하였습니다."), data, Dz1T("JSON"), errp) == FALSE) ERR_OUT(errp);
									else Dz1Error_set(errp, 0);

									if (s->validate_set->x.technical_regulation1_clientRole1->test_pub_roadway_surface_status)
									{
										if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_roadway_surface_status, Dz1T("SUCCESS"), errp) == FALSE) ERR_OUT(errp);
										else Dz1Error_set(errp, 0);
									}
									pthread_cleanup_pop(1); // (Dz1Binary_delAndSetNull, (void*)&data);
								}
							...
							pthread_cleanup_pop(1); // (RoadwaySurfaceStatusList_delAndSetNull, (void*)&list);
						}
						pthread_cleanup_pop(1); // (Dz1RoadwaySurfaceStatusResponse_delAndSetNull, (void*)&rsp);
					}
				}
				else if (Dz1STRCMP(Dz1T("1.0.14827.1.0.6"), msg->oidStr) == 0)
				{
					Dz1WeatherInformationResponse* rsp = NULL;
					if ((rsp = (Dz1WeatherInformationResponse*)api->struct_decode(&bin, arg->initial_codec, errp)) == NULL) ERR_OUT(errp);
					else
					{
						WeatherInformationList* list = NULL;
						pthread_cleanup_push(Dz1WeatherInformationResponse_delAndSetNull, (void*)&rsp);
						if ((list = WeatherInformationList_fromDimz(rsp, errp)) == NULL) ERR_OUT(errp);
						else
						{
							pthread_cleanup_push(WeatherInformationList_delAndSetNull, (void*)&list);
							if ((str_u8 = _get_stringify_u8(env->spec, Dz1T("WeatherInformationList"), list, errp)) == NULL) ERR_OUT(errp);
							else
							{
								pthread_cleanup_push(Dz1Asn1UTF8Str_delAndSetNull, (void*)&str_u8);
								if ((data = Dz1Binary_new(str_u8->data, str_u8->size, errp)) == NULL) ERR_OUT(errp);
								...
								{
									pthread_cleanup_push(Dz1Binary_delAndSetNull, (void*)&data);
									if (Iso14827TestHelperEnvUtil_notifyInfo(env, test_name_test_pub_current_link_state, NULL, Dz1T("기상정보 정보 제공을 확인하였습니다."), data, Dz1T("JSON"), errp) == FALSE) ERR_OUT(errp);
									else Dz1Error_set(errp, 0);

									if (s->validate_set->x.technical_regulation1_clientRole1->test_pub_weather_information)
									{
										if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_weather_information, Dz1T("SUCCESS"), errp) == FALSE) ERR_OUT(errp);
										else Dz1Error_set(errp, 0);
									}
									pthread_cleanup_pop(1); // (Dz1Binary_delAndSetNull, (void*)&data);
								}
							...
							pthread_cleanup_pop(1); // (WeatherInformationList_delAndSetNull, (void*)&list);
						}
						pthread_cleanup_pop(1); // (Dz1WeatherInformationResponse_delAndSetNull, (void*)&rsp);
					}
				}
				else if (Dz1STRCMP(Dz1T("1.0.14827.1.0.7"), msg->oidStr) == 0)
				{
					Dz1LinkRoadwayGeometryResponse* rsp = NULL;
					if ((rsp = (Dz1LinkRoadwayGeometryResponse*)api->struct_decode(&bin, arg->initial_codec, errp)) == NULL) ERR_OUT(errp);
					else
					{
						LinkRoadwayGeometryList* list = NULL;
						pthread_cleanup_push(Dz1LinkRoadwayGeometryResponse_delAndSetNull, (void*)&rsp);
						if ((list = LinkRoadwayGeometryList_fromDimz(rsp, errp)) == NULL) ERR_OUT(errp);
						else
						{
							pthread_cleanup_push(LinkRoadwayGeometryList_delAndSetNull, (void*)&list);
							if ((str_u8 = _get_stringify_u8(env->spec, Dz1T("LinkRoadwayGeometryList"), list, errp)) == NULL) ERR_OUT(errp);
							else
							{
								pthread_cleanup_push(Dz1Asn1UTF8Str_delAndSetNull, (void*)&str_u8);
								if ((data = Dz1Binary_new(str_u8->data, str_u8->size, errp)) == NULL) ERR_OUT(errp);
								...
								{
									pthread_cleanup_push(Dz1Binary_delAndSetNull, (void*)&data);
									if (Iso14827TestHelperEnvUtil_notifyInfo(env, test_name_test_pub_current_link_state, NULL, Dz1T("도로관리정보 정보 제공을 확인하였습니다."), data, Dz1T("JSON"), errp) == FALSE) ERR_OUT(errp);
									else Dz1Error_set(errp, 0);

									if (s->validate_set->x.technical_regulation1_clientRole1->test_pub_link_roadway_geometry)
									{
										if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_link_roadway_geometry, Dz1T("SUCCESS"), errp) == FALSE) ERR_OUT(errp);
										else Dz1Error_set(errp, 0);
									}
									pthread_cleanup_pop(1); // (Dz1Binary_delAndSetNull, (void*)&data);
								}
							...
							pthread_cleanup_pop(1); // (LinkRoadwayGeometryList_delAndSetNull, (void*)&list);
						}
						pthread_cleanup_pop(1); // (Dz1LinkRoadwayGeometryResponse_delAndSetNull, (void*)&rsp);
					}
				}
				else if (Dz1STRCMP(Dz1T("1.0.14827.1.0.8"), msg->oidStr) == 0)
				{
					Dz1ProbeVehicleDetectionResponse* rsp = NULL;
					if ((rsp = (Dz1ProbeVehicleDetectionResponse*)api->struct_decode(&bin, arg->initial_codec, errp)) == NULL) ERR_OUT(errp);
					else
					{
						ProbeVehicleDetectionList* list = NULL;
						pthread_cleanup_push(Dz1ProbeVehicleDetectionResponse_delAndSetNull, (void*)&rsp);
						if ((list = ProbeVehicleDetectionList_fromDimz(rsp, errp)) == NULL) ERR_OUT(errp);
						else
						{
							pthread_cleanup_push(ProbeVehicleDetectionList_delAndSetNull, (void*)&list);
							if ((str_u8 = _get_stringify_u8(env->spec, Dz1T("ProbeVehicleDetectionList"), list, errp)) == NULL) ERR_OUT(errp);
							else
							{
								pthread_cleanup_push(Dz1Asn1UTF8Str_delAndSetNull, (void*)&str_u8);
								if ((data = Dz1Binary_new(str_u8->data, str_u8->size, errp)) == NULL) ERR_OUT(errp);
								...
								{
									pthread_cleanup_push(Dz1Binary_delAndSetNull, (void*)&data);
									if (Iso14827TestHelperEnvUtil_notifyInfo(env, test_name_test_pub_current_link_state, NULL, Dz1T("프로브정보 정보 제공을 확인하였습니다."), data, Dz1T("JSON"), errp) == FALSE) ERR_OUT(errp);
									else Dz1Error_set(errp, 0);

									if (s->validate_set->x.technical_regulation1_clientRole1->test_pub_probe_vehicle_detection)
									{
										if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_probe_vehicle_detection, Dz1T("SUCCESS"), errp) == FALSE) ERR_OUT(errp);
										else Dz1Error_set(errp, 0);
									}
									pthread_cleanup_pop(1); // (Dz1Binary_delAndSetNull, (void*)&data);
								}
							...
							pthread_cleanup_pop(1); // (ProbeVehicleDetectionList_delAndSetNull, (void*)&list);
						}

						pthread_cleanup_pop(1); // (Dz1ProbeVehicleDetectionResponse_delAndSetNull, (void*)&rsp);
					}
				}
				else if (Dz1STRCMP(Dz1T("1.0.14827.1.0.9"), msg->oidStr) == 0)
				{
					Dz1DetectorCollectionResponse* rsp = NULL;
					if ((rsp = (Dz1DetectorCollectionResponse*)api->struct_decode(&bin, arg->initial_codec, errp)) == NULL) ERR_OUT(errp);
					else
					{
						DetectorCollectionList* list = NULL;
						pthread_cleanup_push(Dz1DetectorCollectionResponse_delAndSetNull, (void*)&rsp);
						if ((list = DetectorCollectionList_fromDimz(rsp, errp)) == NULL) ERR_OUT(errp);
						else
						{
							pthread_cleanup_push(DetectorCollectionList_delAndSetNull, (void*)&list);
							if ((str_u8 = _get_stringify_u8(env->spec, Dz1T("DetectorCollectionList"), list, errp)) == NULL) ERR_OUT(errp);
							else
							{
								pthread_cleanup_push(Dz1Asn1UTF8Str_delAndSetNull, (void*)&str_u8);
								if ((data = Dz1Binary_new(str_u8->data, str_u8->size, errp)) == NULL) ERR_OUT(errp);
								...
								{
									pthread_cleanup_push(Dz1Binary_delAndSetNull, (void*)&data);
									if (Iso14827TestHelperEnvUtil_notifyInfo(env, test_name_test_pub_current_link_state, NULL, Dz1T("차량검지정보 정보 제공을 확인하였습니다."), data, Dz1T("JSON"), errp) == FALSE) ERR_OUT(errp);
									else Dz1Error_set(errp, 0);

									if (s->validate_set->x.technical_regulation1_clientRole1->test_pub_detector_collection)
									{
										if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_detector_collection, Dz1T("SUCCESS"), errp) == FALSE) ERR_OUT(errp);
										else Dz1Error_set(errp, 0);
									}
									pthread_cleanup_pop(1); // (Dz1Binary_delAndSetNull, (void*)&data);
								}
							...
							pthread_cleanup_pop(1); // (DetectorCollectionList_delAndSetNull, (void*)&list);
						}

						pthread_cleanup_pop(1); // (Dz1DetectorCollectionResponse_delAndSetNull, (void*)&rsp);
					}
				}
				else if (Dz1STRCMP(Dz1T("1.0.14827.1.1.0"), msg->oidStr) == 0)
				{
					Dz1ExtentionCommPDUResponse* rsp = NULL;
					if ((rsp = (Dz1ExtentionCommPDUResponse*)api->struct_decode(&bin, arg->initial_codec, errp)) == NULL) ERR_OUT(errp);
					else
					{
						ExtentionCommPDUList* list = NULL;
						pthread_cleanup_push(Dz1ExtentionCommPDUResponse_delAndSetNull, (void*)&rsp);
						if ((list = ExtentionCommPDUList_fromDimz(rsp, errp)) == NULL) ERR_OUT(errp);
						else
						{
							pthread_cleanup_push(ExtentionCommPDUList_delAndSetNull, (void*)&list);
							if ((str_u8 = _get_stringify_u8(env->spec, Dz1T("ExtentionCommPDUList"), list, errp)) == NULL) ERR_OUT(errp);
							else
							{
								pthread_cleanup_push(Dz1Asn1UTF8Str_delAndSetNull, (void*)&str_u8);
								if ((data = Dz1Binary_new(str_u8->data, str_u8->size, errp)) == NULL) ERR_OUT(errp);
								...
								{
									pthread_cleanup_push(Dz1Binary_delAndSetNull, (void*)&data);
									if (Iso14827TestHelperEnvUtil_notifyInfo(env, test_name_test_pub_current_link_state, NULL, Dz1T("확장 교통정보 정보 제공을 확인하였습니다."), data, Dz1T("JSON"), errp) == FALSE) ERR_OUT(errp);
									else Dz1Error_set(errp, 0);

									if (s->validate_set->x.technical_regulation1_clientRole1->test_pub_extention_comm_pdu)
									{
										if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_extention_comm_pdu, Dz1T("SUCCESS"), errp) == FALSE) ERR_OUT(errp);
										else Dz1Error_set(errp, 0);
									}
									pthread_cleanup_pop(1); // (Dz1Binary_delAndSetNull, (void*)&data);
								}
							...
							pthread_cleanup_pop(1); // (ExtentionCommPDUList_delAndSetNull, (void*)&list);

						}
						pthread_cleanup_pop(1); // (Dz1ExtentionCommPDUResponse_delAndSetNull, (void*)&rsp);
					}
				}
				else ERR_SET_OUT(errp, EPERM);
			}
			// 기본교통정보교환 2
			else if (Dz1STRCMP(arg->testScenario, Dz1T("TechincalRegulation2-clientRole1")) == 0)
			{
				if (Dz1STRCMP(Dz1T("1.0.15784.2.0.1"), msg->oidStr) == 0)
				{
					Dz1CommTxPDUResponse* rsp = NULL;
					if ((rsp = (Dz1CommTxPDUResponse*)api->struct_decode(&bin, arg->initial_codec, errp)) == NULL) ERR_OUT(errp);
					else
					{
						CommTxPDUList* list = NULL;
						pthread_cleanup_push(Dz1CommTxPDUResponse_delAndSetNull, (void*)&rsp);
						if ((list = CommTxPDUList_fromDimz(rsp, errp)) == NULL) ERR_OUT(errp);
						else
						{
							pthread_cleanup_push(CommTxPDUList_delAndSetNull, (void*)&list);
							if ((str_u8 = _get_stringify_u8(env->spec, Dz1T("CommTxPDUList"), list, errp)) == NULL) ERR_OUT(errp);
							else
							{
								pthread_cleanup_push(Dz1Asn1UTF8Str_delAndSetNull, (void*)&str_u8);
								if ((data = Dz1Binary_new(str_u8->data, str_u8->size, errp)) == NULL) ERR_OUT(errp);
								...
								{
									pthread_cleanup_push(Dz1Binary_delAndSetNull, (void*)&data);
									if (Iso14827TestHelperEnvUtil_notifyInfo(env, test_name_test_pub_current_link_state, NULL, Dz1T("교통제공정보 제공을 확인하였습니다."), data, Dz1T("JSON"), errp) == FALSE) ERR_OUT(errp);
									else Dz1Error_set(errp, 0);

									if (s->validate_set->x.technical_regulation2_clientRole1->test_pub_comm_tx_pdu)
									{
										if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_comm_tx_pdu, Dz1T("SUCCESS"), errp) == FALSE) ERR_OUT(errp);
										else Dz1Error_set(errp, 0);
									}
									pthread_cleanup_pop(1); // (Dz1Binary_delAndSetNull, (void*)&data);
								}
							...
							pthread_cleanup_pop(1); // (CommTxPDUList_delAndSetNull, (void*)&list);

						}
						pthread_cleanup_pop(1); // (Dz1CommTxPDUResponse_delAndSetNull, (void*)&rsp);
					}
				}
				else if (Dz1STRCMP(Dz1T("1.0.15784.2.0.2"), msg->oidStr) == 0)
				{
					Dz1CommRxPDUResponse* rsp = NULL;
					if ((rsp = (Dz1CommRxPDUResponse*)api->struct_decode(&bin, arg->initial_codec, errp)) == NULL) ERR_OUT(errp);
					else
					{
						CommRxPDUList* list = NULL;
						pthread_cleanup_push(Dz1CommRxPDUResponse_delAndSetNull, (void*)&rsp);
						if ((list = CommRxPDUList_fromDimz(rsp, errp)) == NULL) ERR_OUT(errp);
						else
						{
							pthread_cleanup_push(CommRxPDUList_delAndSetNull, (void*)&list);
							if ((str_u8 = _get_stringify_u8(env->spec, Dz1T("CommRxPDUList"), list, errp)) == NULL) ERR_OUT(errp);
							else
							{
								pthread_cleanup_push(Dz1Asn1UTF8Str_delAndSetNull, (void*)&str_u8);
								if ((data = Dz1Binary_new(str_u8->data, str_u8->size, errp)) == NULL) ERR_OUT(errp);
								...
								{
									pthread_cleanup_push(Dz1Binary_delAndSetNull, (void*)&data);
									if (Iso14827TestHelperEnvUtil_notifyInfo(env, test_name_test_pub_current_link_state, NULL, Dz1T("교통수집정보 정보 제공을 확인하였습니다."), data, Dz1T("JSON"), errp) == FALSE) ERR_OUT(errp);
									else Dz1Error_set(errp, 0);

									if (s->validate_set->x.technical_regulation2_clientRole1->test_pub_comm_rx_pdu)
									{
										if (Iso14827TestHelperEnvUtil_notifyState(env, test_name_test_pub_comm_rx_pdu, Dz1T("SUCCESS"), errp) == FALSE) ERR_OUT(errp);
										else Dz1Error_set(errp, 0);
									}
									pthread_cleanup_pop(1); // (Dz1Binary_delAndSetNull, (void*)&data);
								}
							...
							pthread_cleanup_pop(1); // (CommRxPDUList_delAndSetNull, (void*)&list);

						}
						pthread_cleanup_pop(1); // (Dz1CommRxPDUResponse_delAndSetNull, (void*)&rsp);
					}
				}
			}
...
static bool_t _client1_sub_cancel_test(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, Dz1Error* errp)
{
	Iso14827TestClientAccount* acc = env->primary_acc;
...
static bool_t _client1_canceled_pub_response(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, ItsIso14827ClientEventPublication* msg, Dz1Error* errp)
{
...
static bool_t _client1_deferred_ack_test(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, Dz1Error* errp)
{
	Iso14827TestClientAccount* acc = env->primary_acc;
...
static bool_t _client1_deferred_pub_response(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, ItsIso14827ClientEventPublication* msg, Dz1Error* errp)
{
...
static bool_t _client1_do_terminate(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, Dz1Error* errp)
{
	Iso14827TestClientAccount* acc = env->primary_acc;
...
static bool_t _client1_disconnected(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, Dz1Error* errp)
{
...
*/
bool_t Iso14827TestClient1ScenarioTR_check(Iso14827TestClientRole1TR* s, Dz1Error* err)//ConformanceMsgStructAPI* msg_tbl, Dz1Error* err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
...
static Iso14827TestClientAccount* _get_account1(Iso14827TestClientRole1TR* s)
{
...
static void _client1_timer_cleanup(Iso14827TestHelperEnv* env, void* timer)
{
	...
	Iso14827TestClientRole1TR* s = scenario->x.client1_tr;
...
bool_t Iso14827TestClient1ScenarioTR_prepare(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, Dz1Error* errp)
{
	Iso14827TestClientConnection* c = s->connection;
	Iso14827TestClientPassive* p = NULL;
	Iso14827TestClientActive* a = NULL;
...
static bool_t Iso14827TestClient1ScenarioTR_event(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s,
...
bool_t Iso14827TestClient1ScenarioTR_start(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, bool_t* ret_is_complete, Dz1Error* err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	return Iso14827TestClient1ScenarioTR_event(arg, env, timer, s, Iso14827TestClientEvent_start, NULL, ret_is_complete, err);
}
// Start Scenario : Client1
///////////////////////////////////////////////////////////////////////////////
...
bool_t Iso14827TestClient1ScenarioTR_trigger(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, bool_t* ret_is_complete, Dz1Error* err)
{
	return Iso14827TestClient1ScenarioTR_event(arg, env, timer, s, Iso14827TestClientEvent_trigger, NULL, ret_is_complete, err);
}
// Trigger Scenario : Client1
///////////////////////////////////////////////////////////////////////////////
...
bool_t Iso14827TestClient1ScenarioTR_proc14827(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, Iso14827TestClientEvent ev, void* t14827msg, bool_t* ret_is_complete, Dz1Error* err)
{
	return Iso14827TestClient1ScenarioTR_event(arg, env, timer, s, ev, t14827msg, ret_is_complete, err);
}
// ISO-14827 Message : Client1
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
// Timeout : Client1
bool_t Iso14827TestClient1ScenarioTR_timeout(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, Iso14827TestHelperTimer* tmr, bool_t* ret_is_complete, Dz1Error* err)
{
	return Iso14827TestClient1ScenarioTR_event(arg, env, timer, s, Iso14827TestClientEvent_timeout, tmr, ret_is_complete, err);
}
// Timeout : Client1
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
// Stop : Client1
bool_t Iso14827TestClient1ScenarioTR_stop(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, Dz1Error* err)
...
bool_t Iso14827TestClient1ScenarioTR_validateReceived(Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env, void* timer, Iso14827TestClientRole1TR* s, void* msg, bool_t* ret_is_complete, Dz1Error* err)
{
	return Iso14827TestClient1ScenarioTR_event(arg, env, timer, s, Iso14827TestClientEvent_validate, msg, ret_is_complete, err);
}

void   Iso14827TestClient1ScenarioTR_reset(Iso14827TestClientRole1TR* s, Iso14827TestHelperEnv* env)	// back to prepared status
{
...
void   Iso14827TestClient1ScenarioTR_release(Iso14827TestClientRole1TR* s, Iso14827TestHelperEnv* env)	// release resource
{
	// called when terminate state
	Iso14827TestClient1ScenarioTR_reset(s, env);
	Iso14827TestClientAccount* acc = NULL;
...
void Iso14827TestClientRole1TR_status(Iso14827TestClientRole1TR* s, Iso14827TestHelperArg* arg, Iso14827TestHelperEnv* env)
{
	if (s != NULL)
	{
