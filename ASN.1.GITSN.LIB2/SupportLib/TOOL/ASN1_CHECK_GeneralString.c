#include "ASN1_CHECK_GeneralString.h"


ASN1_CPPLINK int ASN1_CHECK_GeneralString(GeneralString *ValueChecked, ASNUINT8 *ConsChecked)
{
	ASNSTRING DefaultAlphabet;
	ASNUINT8 DefaultString[249];
	ASNUINT16 i = 0;
	ASNUINT16 j = 0;

	for (;i<255;i++,j++)
	{
		if (i>135 && i<144) 
		{
			continue;
		}
		
		DefaultString[j] = (ASNUINT8)i;
	}

	DefaultString[j] = 0;

	DefaultAlphabet.nchar = 248;
	DefaultAlphabet.asnstring = DefaultString;
	
	return ASN1_CHECK_STRING((ASNSTRING *)ValueChecked,ConsChecked,&DefaultAlphabet,1);
}
