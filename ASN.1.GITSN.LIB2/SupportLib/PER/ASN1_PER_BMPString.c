									  ...
									  Asn1ConsInfo *consinfo)
{
	...
	ret = ASN1_PER_Align_Enc_BMPString_Process(ws,value,consinfo,BMP_ABITS);
	if (ret < 0){
		return ret;
	}
	return 0;
										...
										Asn1ConsInfo *consinfo)
{
	ASNINT32 ret = 0;
	consinfo->charSet = NULL;
	ret = ASN1_PER_UNAlign_Enc_BMPString_Process(ws,value,consinfo,BMP_UBITS);
	if (ret < 0){
		return ret;
	}
	return 0;
									  ...
									  Asn1ConsInfo *consinfo)
{
	ASNINT32 ret;
	consinfo->charSet = NULL;
	ret = ASN1_PER_Align_Dec_BMPString_Process(ws,value,consinfo,BMP_ABITS);
	if (ret<0){
		return ret;
	}
	return 0;
										...
										Asn1ConsInfo *consinfo)
{
	ASNINT32 ret;
	consinfo->charSet = NULL;
	ret = ASN1_PER_UNAlign_Dec_BMPString_Process(ws,value,consinfo,BMP_UBITS);
	if (ret<0){
		return ret;
	}
	return 0;
...

{
	...
	ASNINT32 ret = 0;
	ASNUINT16* temp_value = value->data;
	...
	if ((per_flag.ext_flag == 1)&&(per_flag.Value_length >= 16384)){
		...
		{	
			...
			ASN1_PER_Enc_BMPString_Value(ws,value,process_length,
	...
	if(per_flag.Range == 1 && per_flag.L_En_Flag == 0){
		...
		ASN1_PER_Enc_BMPString_Value(ws,value,per_flag.Value_length,
										...
										Asn1ConsInfo *consinfo,ASNINT32 Default_bit)
{
	...
	value->data = (ASNUINT16*)mpool_malloc(ws->Pmemory,1*sizeof(ASNUINT16));
	value->nchar = 1;
	...
	if (per_flag.ext_flag == 1){
		...
		{	
			...
			value->data = 
			mpool_realloc(ws->Pmemory,value->data,temp_value_length*sizeof(ASNUINT16));

			if(0>ASN1_PER_Dec_BMPString_Value(ws,value,per_flag.PER_Dec_Len,Default_bit,&per_flag,UNALIGN))
				return ASN1_PER_CHECK_LENGTH_ERROR;
	...
	if(per_flag.Range == 1 && per_flag.L_En_Flag == 0){
		if (consinfo->extflag == 1){
			...
			if (per_flag.ext_flag == 1){
				...
				mpool_realloc(ws->Pmemory,value->data,
							  per_flag.PER_Dec_Len*sizeof(ASNUINT16));
		...
		if(0>ASN1_PER_Dec_BMPString_Value(ws,value,per_flag.PER_Dec_Len,
			Default_bit,&per_flag,align_flag))
			return ASN1_PER_CHECK_LENGTH_ERROR;
	...
	if(per_flag.Is_Size == 1){
		...
		value->nchar = per_flag.PER_Dec_Len;
		value->data =
		mpool_realloc(ws->Pmemory,value->data,per_flag.PER_Dec_Len*sizeof(ASNUINT16));
...
{
	ASNUINT8 bmp_string[2];
	ASNUINT16* encode = value->data;
	
	ASNINT32 i = 0;
	
	memset(bmp_string,0x00,2);
	value->data += length;		

	while (length)
	{
		bmp_string[0] = (ASNUINT8)((*encode)>>8);
		bmp_string[1] = (ASNUINT8)(*encode);

		if (!i){
			ASN1_FillBuffer(ws,bmp_string,Default_bit,Align_Flag);
			i++;
		}
		else{
			ASN1_FillBuffer(ws,bmp_string,Default_bit,UNALIGN);
		}
		encode++;
		length--;
	}
								...
								struct PER_Flag *per_flag,ASN1PERAlign Align_Flag)
{
	...
	ASNUINT16* Tdecode = (value->data + (value->nchar - length));
	...
	{
		if (!i){
			if(0>ASN1_ReadBuffer(ws,bmp_string,Default_bit,Align_Flag))
				return -403;
			i++;
		}
		else{
			if(0>ASN1_ReadBuffer(ws,bmp_string,Default_bit,UNALIGN))
				return -403;
		}
		
		*Tdecode = bmp_string[0];
		*Tdecode<<=8;
		*Tdecode += bmp_string[1];
