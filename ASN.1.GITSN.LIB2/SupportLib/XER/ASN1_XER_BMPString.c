#include "ASN1_XER_BMPString.h"

ASNUINT8 zero_bmp[]= {0xff,0x7f,0x3f,0x1f,0x0f,0x07,0x03,0x01,0x00};
ASNUINT8 one_bmp[] = {0x00,0x80,0xc0,0xe0,0xf0,0xf8,0xfc,0xfe,0xff};

ASN1_CPPLINK void change_uarray_bmp(ASNUINT8 *value,ASNINT32 lenght)
{
...
ASN1_CPPLINK ASNINT32 ASN1_XER_Enc_BMPString_Value(ASN1WorkSpace * ws,BMPString *tvalue)
{
	ASNUINT16 uvalue;
	ASNUINT16 *temp_value = tvalue->data;
	ASNUINT8  uarray[10];
	...
	while(loop_flag){//while end1
		...
		uarray_lenght = 0;
			...
			if(uvalue < 128){
				...
				uarray[uarray_lenght - 1] &= zero_bmp[uarray_lenght];
				uarray[uarray_lenght - 1] |= one_bmp[uarray_lenght];
				change_uarray_bmp(uarray,uarray_lenght);
			}
...
ASN1_CPPLINK ASNINT32 ASN1_XER_BMP_CONV(ASNUINT8 *value)
{
	if (!strcmp(value,"&amp;"))
		return 38;
	if (!strcmp(value,"&lt;"))
...
ASN1_CPPLINK ASNUINT16 ASN1_XER_BMP_DEC_CONV(ASN1WorkSpace * ws)
{
	ASNUINT16 temp_ui = 0;
	...
	if((*ws->buffer.current & 0x80) == 0x00){
		...
		temp_ui = (*ws->buffer.current & zero_bmp[lenght--]);
		ws->buffer.current++;
...
ASN1_CPPLINK ASNINT32 ASN1_XER_Dec_BMPString_Value(ASN1WorkSpace * ws, BMPString *value)
{
	...
	value->data = mpool_malloc(ws->Pmemory,memory*sizeof(ASNUINT16));
	value->nchar = 0;
	...
	while(!(*ws->buffer.current == '<'|| *ws->buffer.current == '/')) {
		if(*ws->buffer.current == '&'){
			...
			value->data = mpool_realloc(ws->Pmemory,
										value->data,
										++memory*sizeof(ASNUINT16));
			value->data[k++] = ASN1_XER_BMP_CONV(temp_value);
			value->nchar++;
			j=0;
			memset(temp_value,0x00,10);
		}
			...
			value->data = mpool_realloc(ws->Pmemory,
										value->data,
										++memory*sizeof(ASNUINT16));
			value->data[k++] = ASN1_XER_BMP_DEC_CONV(ws);
			value->nchar++;
		}
	}
										  ...
										  ASNUINT8 *name,ASNINT32 tab_value)
{
	...
	ASN1_XER_Enc_BMPString_Value(ws,tvalue);
											...
											ASNUINT8 *name,ASNINT32 tab_value)
{
	...
	ASN1_XER_CANONICAL_Enc_BMPString(ws,tvalue,name,tab_value);
	ASN1_XER_Enter(ws,1);
	return 0;
}
												...
												ASNUINT8 *name,ASNINT32 tab_value)
{
	...
	ASN1_XER_Dec_BMPString_Value(ws,value);
											...
											ASNUINT8 *name,ASNINT32 tab_value)
{
	...
	ASN1_XER_CANONICAL_Dec_BMPString(ws,value,name,tab_value);
/*	ASN1_XER_Dec_Format(ws);*/
	return 0;
}
