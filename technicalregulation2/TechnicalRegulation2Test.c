...
//#include <TechnicalRegulation2.h>
...
#ifndef UNIX_SYSTEM
	if ((sh = Dz1Shell_new(Dz1Text("win32crt"), DZ1_SHELL_CMD_ALL, 16, Dz1Text("root"), Dz1Text("TechnicalRegulation2"), errp)) == NULL) ERR_OUT(errp);
#else
	if ((sh = Dz1Shell_new(Dz1Text("xterm"), DZ1_SHELL_CMD_ALL, 16, Dz1Text("root"), Dz1Text("TechnicalRegulation2"), errp)) == NULL) ERR_OUT(errp);
#endif
	else
	{
...
#endif // UNIC_SYSTEM
{
	...
	if ((logfp = Dz1FileStream_open(Dz1Text("TechnicalRegulation2Test.log.txt"), Dz1Text("wb"), NULL)) != NULL)
	{
