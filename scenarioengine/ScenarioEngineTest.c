...
#ifdef WIN32
	if ((sh = Dz1Shell_new(Dz1Text("win32crt"), DZ1_SHELL_CMD_ALL, 16, Dz1Text("root"), Dz1Text("ScenarioEngine"), errp)) == NULL) ERR_OUT(errp);
#else
	if ((sh = Dz1Shell_new(Dz1Text("xterm"), DZ1_SHELL_CMD_ALL, 16, Dz1Text("root"), Dz1Text("ScenarioEngine"), errp)) == NULL) ERR_OUT(errp);
#endif
	else
	{
...
{
	Dz1Thread_log(DZ1_THREAD_LOG_ERROR, "ScenarioEngine : Exception Catch\n");
	ERR_OUT(reason);
}
...
static bool_t _main_start(bool_t is_daemon, Dz1Error* errp)
{
	...
	else if ((env.task = ScenarioEngine_new(env.server_uri, _task_exception_handler, NULL, errp)) == NULL) ERR_OUT(errp);	// Edit Task Argument
	else if ((*errp = Dz1Task_start(env.task, DZ1_THREAD_LOG_BASIS | DZ1_THREAD_LOG_TASK_DEBUG | DZ1_THREAD_LOG_DEBUG)).code) ERR_OUT(errp);
	else
	{
...
static bool_t _process_start(bool_t is_daemon, Dz1Error *errp)
{
	...
	if ((logfp = Dz1FileStream_open(Dz1Text("ScenarioEngineTest.log.txt"), Dz1Text("wb"), NULL)) != NULL)
	{
...
#endif // UNIC_SYSTEM
{
	...
	if (argc != 2 && argc != 3)
	{
		...
		fprintf(stderr, Dz1T("%s <WebSocketServer> [-debug]\n"), bn);
	}
	else
	{
		bool_t daemon_mode = TRUE;
		server_uri = argv[1];
		const char* prefix = "scts://";
		if (strncmp(argv[1], prefix, strlen(prefix)) == 0) {
			server_uri = argv[1] + strlen(prefix);
		}
		// server_uri = argv[1];
		if (argc == 3 && strcmp(argv[2], Dz1T("-debug")) == 0) daemon_mode = FALSE;
