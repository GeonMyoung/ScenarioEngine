...
////////////////////////////////////////////////////////////////////////////////
// Gitsn_ResponseType
static bool_t Gitsn_ResponseType_load(struct Dz1JsonVarBind *dst_obj, Dz1JsonToken *tok, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	Gitsn_ResponseType v = Gitsn_ResponseType_max;
	switch(tok->t)
	{
	case Dz1JsonTokenType_number:
		v = (Gitsn_ResponseType)Dz1StrTo32(tok->v);
		break;
	case Dz1JsonTokenType_string:
		if ((v = Gitsn_ResponseTypeFromStr(tok->v)) == Gitsn_ResponseType_max) ERR_SET_OUT_RET(errp, EPERM, FALSE);
		break;
	default: ERR_SET_RET(errp, EPERM, FALSE);
	}

	if ((dst_obj->data = Gitsn_ResponseType_new(&v, errp)) == NULL) ERR_OUT(errp);
	else Dz1Error_set(errp, 0);

	return errp->code == 0 ? TRUE : FALSE;
}

static bool_t Gitsn_ResponseType_stringify(void *obj, Dz1ElasticBuf *buf, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	if (obj == NULL)
	{
	...
	{
		Gitsn_ResponseType v = *(Gitsn_ResponseType *)obj;
		Dz1Str str_v = Gitsn_ResponseTypeStr(v);
		if (str_v == NULL)
		{
			if (Dz1JsonUtil_stringify_u32(buf, (u32_t)v, errp) == FALSE) ERR_OUT(errp);
			else Dz1Error_set(errp, 0);
...
////////////////////////////////////////////////////////////////////////////////
// Gitsn_SirenInUse
static bool_t Gitsn_SirenInUse_load(struct Dz1JsonVarBind *dst_obj, Dz1JsonToken *tok, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	Gitsn_SirenInUse v = Gitsn_SirenInUse_max;
	switch(tok->t)
	{
	case Dz1JsonTokenType_number:
		v = (Gitsn_SirenInUse)Dz1StrTo32(tok->v);
		break;
	case Dz1JsonTokenType_string:
		if ((v = Gitsn_SirenInUseFromStr(tok->v)) == Gitsn_SirenInUse_max) ERR_SET_OUT_RET(errp, EPERM, FALSE);
		break;
	default: ERR_SET_RET(errp, EPERM, FALSE);
	}

	if ((dst_obj->data = Gitsn_SirenInUse_new(&v, errp)) == NULL) ERR_OUT(errp);
	else Dz1Error_set(errp, 0);

	return errp->code == 0 ? TRUE : FALSE;
}

static bool_t Gitsn_SirenInUse_stringify(void *obj, Dz1ElasticBuf *buf, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	if (obj == NULL)
	{
	...
	{
		Gitsn_SirenInUse v = *(Gitsn_SirenInUse *)obj;
		Dz1Str str_v = Gitsn_SirenInUseStr(v);
		if (str_v == NULL)
		{
			if (Dz1JsonUtil_stringify_u32(buf, (u32_t)v, errp) == FALSE) ERR_OUT(errp);
			else Dz1Error_set(errp, 0);
...
static bool_t Gitsn_MultiVehicleResponse_load(struct Dz1JsonVarBind *dst_obj, Dz1JsonToken *tok, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	Gitsn_MultiVehicleResponse v = Gitsn_MultiVehicleResponse_max;
	switch(tok->t)
	{
	case Dz1JsonTokenType_number:
		v = (Gitsn_MultiVehicleResponse)Dz1StrTo32(tok->v);
		break;
	case Dz1JsonTokenType_string:
		if ((v = Gitsn_MultiVehicleResponseFromStr(tok->v)) == Gitsn_MultiVehicleResponse_max) ERR_SET_OUT_RET(errp, EPERM, FALSE);
		break;
	default: ERR_SET_RET(errp, EPERM, FALSE);
	}

	if ((dst_obj->data = Gitsn_MultiVehicleResponse_new(&v, errp)) == NULL) ERR_OUT(errp);
	else Dz1Error_set(errp, 0);

	return errp->code == 0 ? TRUE : FALSE;
}

static bool_t Gitsn_MultiVehicleResponse_stringify(void *obj, Dz1ElasticBuf *buf, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	if (obj == NULL)
	{
	...
	{
		Gitsn_MultiVehicleResponse v = *(Gitsn_MultiVehicleResponse *)obj;
		Dz1Str str_v = Gitsn_MultiVehicleResponseStr(v);
		if (str_v == NULL)
		{
			if (Dz1JsonUtil_stringify_u32(buf, (u32_t)v, errp) == FALSE) ERR_OUT(errp);
			else Dz1Error_set(errp, 0);
...
static bool_t Gitsn_PrivilegedEvents_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	switch(id->index)
	{
	case 0: p->doNotUse =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 1: u8_t_delAndSetNull(&p->event); p->event = (u8_t *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_PrivilegedEvents_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_PrivilegedEvents *p = (Gitsn_PrivilegedEvents *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	{
	case 0: return &p->doNotUse;
	case 1: return p->event;
	default: return NULL;
	}
}

static Dz1JsonSpecChild Gitsn_PrivilegedEvents_membs[] =
{
	{ Dz1Text("doNotUse"), Dz1Text("s64_t"), 0 },
	{ Dz1Text("event"), Dz1Text("u8_t"), 1 },
	{ NULL, NULL, -1 }
};
// Gitsn_PrivilegedEvents
////////////////////////////////////////////////////////////////////////////////
...
static bool_t Gitsn_EmergencyDetails_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	{
	case 0: p->doNotUse =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 1: p->sirenUse =  *(Gitsn_SirenInUse *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 2: p->lightsUse =  *(Gitsn_LightbarInUse *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 3: p->multi =  *(Gitsn_MultiVehicleResponse *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 4: Gitsn_PrivilegedEvents_delAndSetNull(&p->events); p->events = (Gitsn_PrivilegedEvents *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 5: p->responseType =  *(Gitsn_ResponseType *)info->data; break; // type_scalar = 1, ptr_instance = 0
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_EmergencyDetails_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_EmergencyDetails *p = (Gitsn_EmergencyDetails *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	{
	case 0: return &p->doNotUse;
	case 1: return &p->sirenUse;
	case 2: return &p->lightsUse;
	case 3: return &p->multi;
	case 4: return p->events;
	case 5: return &p->responseType;
	default: return NULL;
	}
}

static Dz1JsonSpecChild Gitsn_EmergencyDetails_membs[] =
{
	{ Dz1Text("doNotUse"), Dz1Text("s64_t"), 0 },
	{ Dz1Text("sirenUse"), Dz1Text("Gitsn_SirenInUse"), 1 },
	{ Dz1Text("lightsUse"), Dz1Text("Gitsn_LightbarInUse"), 2 },
	{ Dz1Text("multi"), Dz1Text("Gitsn_MultiVehicleResponse"), 3 },
	{ Dz1Text("events"), Dz1Text("Gitsn_PrivilegedEvents"), 4 },
	{ Dz1Text("responseType"), Dz1Text("Gitsn_ResponseType"), 5 },
	{ NULL, NULL, -1 }
};
// Gitsn_EmergencyDetails
////////////////////////////////////////////////////////////////////////////////
...
static bool_t Gitsn_EmergencyVehicleAlert_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	case 0: p->timeStamp =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 1: Dz1Str_delAndSetNull(&p->id); p->id = (Dz1Str)info->data; info->data = NULL; break; // type_scalar = 0, ptr_instance = 0
	case 2: Gitsn_RoadSideAlert_delAndSetNull(&p->rsaMsg); p->rsaMsg = (Gitsn_RoadSideAlert *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 3: p->responseType =  *(Gitsn_ResponseType *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 4: Gitsn_EmergencyDetails_delAndSetNull(&p->details); p->details = (Gitsn_EmergencyDetails *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 5: p->mass =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 6: p->basicType =  *(Gitsn_VehicleType *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 7: p->vehicleType =  *(Gitsn_VehicleGroupAffected *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 8: p->responseEquip =  *(Gitsn_IncidentResponseEquipment *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 9: p->responderType =  *(Gitsn_ResponderGroupAffected *)info->data; break; // type_scalar = 1, ptr_instance = 0
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_EmergencyVehicleAlert_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_EmergencyVehicleAlert *p = (Gitsn_EmergencyVehicleAlert *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	...
	case 0: return &p->timeStamp;
	case 1: return p->id;
	case 2: return p->rsaMsg;
	case 3: return &p->responseType;
	case 4: return p->details;
	case 5: return &p->mass;
	case 6: return &p->basicType;
	case 7: return &p->vehicleType;
	case 8: return &p->responseEquip;
	case 9: return &p->responderType;
	default: return NULL;
	}
}

static Dz1JsonSpecChild Gitsn_EmergencyVehicleAlert_membs[] =
{
	{ Dz1Text("timeStamp"), Dz1Text("s64_t"), 0 },
	{ Dz1Text("id"), Dz1Text("Dz1Str"), 1 },
	{ Dz1Text("rsaMsg"), Dz1Text("Gitsn_RoadSideAlert"), 2 },
	{ Dz1Text("responseType"), Dz1Text("Gitsn_ResponseType"), 3 },
	{ Dz1Text("details"), Dz1Text("Gitsn_EmergencyDetails"), 4 },
	{ Dz1Text("mass"), Dz1Text("s64_t"), 5 },
	{ Dz1Text("basicType"), Dz1Text("Gitsn_VehicleType"), 6 },
	{ Dz1Text("vehicleType"), Dz1Text("Gitsn_VehicleGroupAffected"), 7 },
	{ Dz1Text("responseEquip"), Dz1Text("Gitsn_IncidentResponseEquipment"), 8 },
	{ Dz1Text("responderType"), Dz1Text("Gitsn_ResponderGroupAffected"), 9 },
	{ NULL, NULL, -1 }
};
// Gitsn_EmergencyVehicleAlert
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
// Gitsn_RTCM_Revision
static bool_t Gitsn_RTCM_Revision_load(struct Dz1JsonVarBind *dst_obj, Dz1JsonToken *tok, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	Gitsn_RTCM_Revision v = Gitsn_RTCM_Revision_max;
	switch(tok->t)
	{
	case Dz1JsonTokenType_number:
		v = (Gitsn_RTCM_Revision)Dz1StrTo32(tok->v);
		break;
	case Dz1JsonTokenType_string:
		if ((v = Gitsn_RTCM_RevisionFromStr(tok->v)) == Gitsn_RTCM_Revision_max) ERR_SET_OUT_RET(errp, EPERM, FALSE);
		break;
	default: ERR_SET_RET(errp, EPERM, FALSE);
	}

	if ((dst_obj->data = Gitsn_RTCM_Revision_new(&v, errp)) == NULL) ERR_OUT(errp);
	else Dz1Error_set(errp, 0);

	return errp->code == 0 ? TRUE : FALSE;
}

static bool_t Gitsn_RTCM_Revision_stringify(void *obj, Dz1ElasticBuf *buf, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	if (obj == NULL)
	{
	...
	{
		Gitsn_RTCM_Revision v = *(Gitsn_RTCM_Revision *)obj;
		Dz1Str str_v = Gitsn_RTCM_RevisionStr(v);
		if (str_v == NULL)
		{
			if (Dz1JsonUtil_stringify_u32(buf, (u32_t)v, errp) == FALSE) ERR_OUT(errp);
			else Dz1Error_set(errp, 0);
...
static bool_t Gitsn_AntennaOffsetSet_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	switch(id->index)
	{
	case 0: p->antOffsetX =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 1: p->antOffsetY =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 2: p->antOffsetZ =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_AntennaOffsetSet_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_AntennaOffsetSet *p = (Gitsn_AntennaOffsetSet *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	{
	case 0: return &p->antOffsetX;
	case 1: return &p->antOffsetY;
	case 2: return &p->antOffsetZ;
	default: return NULL;
	}
}

static Dz1JsonSpecChild Gitsn_AntennaOffsetSet_membs[] =
{
	{ Dz1Text("antOffsetX"), Dz1Text("s64_t"), 0 },
	{ Dz1Text("antOffsetY"), Dz1Text("s64_t"), 1 },
	{ Dz1Text("antOffsetZ"), Dz1Text("s64_t"), 2 },
	{ NULL, NULL, -1 }
};
// Gitsn_AntennaOffsetSet
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
// Gitsn_RTCMheader
static bool_t Gitsn_RTCMheader_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	switch(id->index)
	{
	case 0: u8_t_delAndSetNull(&p->status); p->status = (u8_t *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 1: Gitsn_AntennaOffsetSet_delAndSetNull(&p->offsetSet); p->offsetSet = (Gitsn_AntennaOffsetSet *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_RTCMheader_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_RTCMheader *p = (Gitsn_RTCMheader *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	...
	case 1: return p->offsetSet;
	default: return NULL;
	}
}

static Dz1JsonSpecChild Gitsn_RTCMheader_membs[] =
{
	{ Dz1Text("status"), Dz1Text("u8_t"), 0 },
	{ Dz1Text("offsetSet"), Dz1Text("Gitsn_AntennaOffsetSet"), 1 },
	{ NULL, NULL, -1 }
};
// Gitsn_RTCMheader
////////////////////////////////////////////////////////////////////////////////
...
static bool_t Gitsn_RTCMmessageList_append(void *dst_ptr, Dz1JsonVarBind *var, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	Gitsn_RTCMmessageList *dst = (Gitsn_RTCMmessageList *)dst_ptr;
	Dz1Str node = NULL;
...
static u32_t Gitsn_RTCMmessageList_cnt(void *ptr)
{
	u32_t ret = 0;
	Gitsn_RTCMmessageList *list = (Gitsn_RTCMmessageList *)ptr;
	if (list != NULL) ret = list->count(list);
	return ret;
}

static Dz1Error Gitsn_RTCMmessageList_tlv(void *obj, Dz1JsonArrayEntryF f, void *arg)
{
	DZ1_ERROR_SAFE_VAR(errp, err);
	Gitsn_RTCMmessageList *list = (Gitsn_RTCMmessageList *)obj;
	if (list == NULL) ERR_SET_OUT(errp, EINVAL);
	else if ((*errp = list->travel(list, f, arg)).code) ERR_OUT(errp);
	else Dz1Error_set(errp, 0);
	return err;
...
static bool_t Gitsn_RTCMcorrections_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	{
	case 0: p->msgCnt =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 1: p->rev =  *(Gitsn_RTCM_Revision *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 2: p->timeStamp =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 3: Gitsn_FullPositionVector_delAndSetNull(&p->anchorPoint); p->anchorPoint = (Gitsn_FullPositionVector *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 4: Gitsn_RTCMheader_delAndSetNull(&p->rtcmHeader); p->rtcmHeader = (Gitsn_RTCMheader *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 5: Gitsn_RTCMmessageList_delAndSetNull(&p->msgs); p->msgs = (Gitsn_RTCMmessageList *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_RTCMcorrections_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_RTCMcorrections *p = (Gitsn_RTCMcorrections *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	...
	case 3: return p->anchorPoint;
	case 4: return p->rtcmHeader;
	case 5: return p->msgs;
	default: return NULL;
	}
}

static Dz1JsonSpecChild Gitsn_RTCMcorrections_membs[] =
{
	{ Dz1Text("msgCnt"), Dz1Text("s64_t"), 0 },
	{ Dz1Text("rev"), Dz1Text("Gitsn_RTCM_Revision"), 1 },
	{ Dz1Text("timeStamp"), Dz1Text("s64_t"), 2 },
	{ Dz1Text("anchorPoint"), Dz1Text("Gitsn_FullPositionVector"), 3 },
	{ Dz1Text("rtcmHeader"), Dz1Text("Gitsn_RTCMheader"), 4 },
	{ Dz1Text("msgs"), Dz1Text("Gitsn_RTCMmessageList"), 5 },
	{ NULL, NULL, -1 }
};
// Gitsn_RTCMcorrections
////////////////////////////////////////////////////////////////////////////////
...
static bool_t Gitsn_PriorityRequestType_load(struct Dz1JsonVarBind *dst_obj, Dz1JsonToken *tok, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	Gitsn_PriorityRequestType v = Gitsn_PriorityRequestType_max;
	switch(tok->t)
	{
	case Dz1JsonTokenType_number:
		v = (Gitsn_PriorityRequestType)Dz1StrTo32(tok->v);
		break;
	case Dz1JsonTokenType_string:
		if ((v = Gitsn_PriorityRequestTypeFromStr(tok->v)) == Gitsn_PriorityRequestType_max) ERR_SET_OUT_RET(errp, EPERM, FALSE);
		break;
	default: ERR_SET_RET(errp, EPERM, FALSE);
	}

	if ((dst_obj->data = Gitsn_PriorityRequestType_new(&v, errp)) == NULL) ERR_OUT(errp);
	else Dz1Error_set(errp, 0);

	return errp->code == 0 ? TRUE : FALSE;
}

static bool_t Gitsn_PriorityRequestType_stringify(void *obj, Dz1ElasticBuf *buf, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	if (obj == NULL)
	{
	...
	{
		Gitsn_PriorityRequestType v = *(Gitsn_PriorityRequestType *)obj;
		Dz1Str str_v = Gitsn_PriorityRequestTypeStr(v);
		if (str_v == NULL)
		{
			if (Dz1JsonUtil_stringify_u32(buf, (u32_t)v, errp) == FALSE) ERR_OUT(errp);
			else Dz1Error_set(errp, 0);
...
static bool_t Gitsn_IntersectionAccessPoint_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	switch(id->index)
	{
	case Gitsn_IntersectionAccessPointPresent_lane: p->x.lane = *(s64_t *)info->data; break;
	case Gitsn_IntersectionAccessPointPresent_approach: p->x.approach = *(s64_t *)info->data; break;
	case Gitsn_IntersectionAccessPointPresent_connection: p->x.connection = *(s64_t *)info->data; break;
	default: ERR_SET_OUT_RET(errp, ENOENT, FALSE);
	}
	p->present = (Gitsn_IntersectionAccessPointPresent)id->index;
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_IntersectionAccessPoint_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_IntersectionAccessPoint *p = (Gitsn_IntersectionAccessPoint *)ptr;
	Dz1JsonSpecChild *id = (*info) = Dz1JsonSpecChilds_findByIdx(lists, p->present);
	if (id != NULL)
	{
		switch(p->present)
		{
		case Gitsn_IntersectionAccessPointPresent_lane: return &p->x.lane;
		case Gitsn_IntersectionAccessPointPresent_approach: return &p->x.approach;
		case Gitsn_IntersectionAccessPointPresent_connection: return &p->x.connection;
		default: return NULL;
		}
	}
	else return NULL;
...
static Dz1JsonSpecChild Gitsn_IntersectionAccessPoint_membs[] =
{
	{ Dz1Text("lane"), Dz1Text("s64_t"), Gitsn_IntersectionAccessPointPresent_lane },
	{ Dz1Text("approach"), Dz1Text("s64_t"), Gitsn_IntersectionAccessPointPresent_approach },
	{ Dz1Text("connection"), Dz1Text("s64_t"), Gitsn_IntersectionAccessPointPresent_connection },
	{ NULL, NULL, -1 }
};
// Gitsn_IntersectionAccessPoint
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
// Gitsn_SignalRequest
static bool_t Gitsn_SignalRequest_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	switch(id->index)
	{
	case 0: Gitsn_IntersectionReferenceID_delAndSetNull(&p->id); p->id = (Gitsn_IntersectionReferenceID *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 1: p->requestID =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 2: p->requestType =  *(Gitsn_PriorityRequestType *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 3: Gitsn_IntersectionAccessPoint_delAndSetNull(&p->inBoundLane); p->inBoundLane = (Gitsn_IntersectionAccessPoint *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 4: Gitsn_IntersectionAccessPoint_delAndSetNull(&p->outBoundLane); p->outBoundLane = (Gitsn_IntersectionAccessPoint *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_SignalRequest_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_SignalRequest *p = (Gitsn_SignalRequest *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	{
	case 0: return p->id;
	case 1: return &p->requestID;
	case 2: return &p->requestType;
	case 3: return p->inBoundLane;
	case 4: return p->outBoundLane;
	default: return NULL;
	}
}

static Dz1JsonSpecChild Gitsn_SignalRequest_membs[] =
{
	{ Dz1Text("id"), Dz1Text("Gitsn_IntersectionReferenceID"), 0 },
	{ Dz1Text("requestID"), Dz1Text("s64_t"), 1 },
	{ Dz1Text("requestType"), Dz1Text("Gitsn_PriorityRequestType"), 2 },
	{ Dz1Text("inBoundLane"), Dz1Text("Gitsn_IntersectionAccessPoint"), 3 },
	{ Dz1Text("outBoundLane"), Dz1Text("Gitsn_IntersectionAccessPoint"), 4 },
	{ NULL, NULL, -1 }
};
// Gitsn_SignalRequest
////////////////////////////////////////////////////////////////////////////////
...
static bool_t Gitsn_SignalRequestPackage_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	switch(id->index)
	{
	case 0: Gitsn_SignalRequest_delAndSetNull(&p->request); p->request = (Gitsn_SignalRequest *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 1: p->minute =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 2: p->second =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 3: p->duration =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_SignalRequestPackage_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_SignalRequestPackage *p = (Gitsn_SignalRequestPackage *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	{
	case 0: return p->request;
	case 1: return &p->minute;
	case 2: return &p->second;
	case 3: return &p->duration;
	default: return NULL;
	}
}

static Dz1JsonSpecChild Gitsn_SignalRequestPackage_membs[] =
{
	{ Dz1Text("request"), Dz1Text("Gitsn_SignalRequest"), 0 },
	{ Dz1Text("minute"), Dz1Text("s64_t"), 1 },
	{ Dz1Text("second"), Dz1Text("s64_t"), 2 },
	{ Dz1Text("duration"), Dz1Text("s64_t"), 3 },
	{ NULL, NULL, -1 }
};
// Gitsn_SignalRequestPackage
////////////////////////////////////////////////////////////////////////////////
...
static bool_t Gitsn_SignalRequestList_append(void *dst_ptr, Dz1JsonVarBind *var, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	Gitsn_SignalRequestList *dst = (Gitsn_SignalRequestList *)dst_ptr;
	Gitsn_SignalRequestPackage *node = NULL;
	if (dst == NULL || var == NULL || (node = (Gitsn_SignalRequestPackage *)var->data) == NULL) ERR_SET_OUT(errp, EINVAL);
	else if ((*errp = dst->add(dst, node)).code) ERR_OUT(errp);
	else
	{
		var->data = NULL;
...
static u32_t Gitsn_SignalRequestList_cnt(void *ptr)
{
	u32_t ret = 0;
	Gitsn_SignalRequestList *list = (Gitsn_SignalRequestList *)ptr;
	if (list != NULL) ret = list->count(list);
	return ret;
}

static Dz1Error Gitsn_SignalRequestList_tlv(void *obj, Dz1JsonArrayEntryF f, void *arg)
{
	DZ1_ERROR_SAFE_VAR(errp, err);
	Gitsn_SignalRequestList *list = (Gitsn_SignalRequestList *)obj;
	if (list == NULL) ERR_SET_OUT(errp, EINVAL);
	else if ((*errp = list->travel(list, f, arg)).code) ERR_OUT(errp);
	else Dz1Error_set(errp, 0);
	return err;
...
static bool_t Gitsn_RequestSubRole_load(struct Dz1JsonVarBind *dst_obj, Dz1JsonToken *tok, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	Gitsn_RequestSubRole v = Gitsn_RequestSubRole_max;
	switch(tok->t)
	{
	case Dz1JsonTokenType_number:
		v = (Gitsn_RequestSubRole)Dz1StrTo32(tok->v);
		break;
	case Dz1JsonTokenType_string:
		if ((v = Gitsn_RequestSubRoleFromStr(tok->v)) == Gitsn_RequestSubRole_max) ERR_SET_OUT_RET(errp, EPERM, FALSE);
		break;
	default: ERR_SET_RET(errp, EPERM, FALSE);
	}

	if ((dst_obj->data = Gitsn_RequestSubRole_new(&v, errp)) == NULL) ERR_OUT(errp);
	else Dz1Error_set(errp, 0);

	return errp->code == 0 ? TRUE : FALSE;
}

static bool_t Gitsn_RequestSubRole_stringify(void *obj, Dz1ElasticBuf *buf, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	if (obj == NULL)
	{
	...
	{
		Gitsn_RequestSubRole v = *(Gitsn_RequestSubRole *)obj;
		Dz1Str str_v = Gitsn_RequestSubRoleStr(v);
		if (str_v == NULL)
		{
			if (Dz1JsonUtil_stringify_u32(buf, (u32_t)v, errp) == FALSE) ERR_OUT(errp);
			else Dz1Error_set(errp, 0);
...
static bool_t Gitsn_RequestImportanceLevel_load(struct Dz1JsonVarBind *dst_obj, Dz1JsonToken *tok, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	Gitsn_RequestImportanceLevel v = Gitsn_RequestImportanceLevel_max;
	switch(tok->t)
	{
	case Dz1JsonTokenType_number:
		v = (Gitsn_RequestImportanceLevel)Dz1StrTo32(tok->v);
		break;
	case Dz1JsonTokenType_string:
		if ((v = Gitsn_RequestImportanceLevelFromStr(tok->v)) == Gitsn_RequestImportanceLevel_max) ERR_SET_OUT_RET(errp, EPERM, FALSE);
		break;
	default: ERR_SET_RET(errp, EPERM, FALSE);
	}

	if ((dst_obj->data = Gitsn_RequestImportanceLevel_new(&v, errp)) == NULL) ERR_OUT(errp);
	else Dz1Error_set(errp, 0);

	return errp->code == 0 ? TRUE : FALSE;
}

static bool_t Gitsn_RequestImportanceLevel_stringify(void *obj, Dz1ElasticBuf *buf, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	if (obj == NULL)
	{
	...
	{
		Gitsn_RequestImportanceLevel v = *(Gitsn_RequestImportanceLevel *)obj;
		Dz1Str str_v = Gitsn_RequestImportanceLevelStr(v);
		if (str_v == NULL)
		{
			if (Dz1JsonUtil_stringify_u32(buf, (u32_t)v, errp) == FALSE) ERR_OUT(errp);
			else Dz1Error_set(errp, 0);
...
////////////////////////////////////////////////////////////////////////////////
// Gitsn_RequestorType
static bool_t Gitsn_RequestorType_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	switch(id->index)
	{
	case 0: p->role =  *(Gitsn_BasicVehicleRole *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 1: p->subrole =  *(Gitsn_RequestSubRole *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 2: p->request =  *(Gitsn_RequestImportanceLevel *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 3: p->iso3883 =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 4: p->hpmsType =  *(Gitsn_VehicleType *)info->data; break; // type_scalar = 1, ptr_instance = 0
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_RequestorType_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_RequestorType *p = (Gitsn_RequestorType *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	{
	case 0: return &p->role;
	case 1: return &p->subrole;
	case 2: return &p->request;
	case 3: return &p->iso3883;
	case 4: return &p->hpmsType;
	default: return NULL;
	}
}

static Dz1JsonSpecChild Gitsn_RequestorType_membs[] =
{
	{ Dz1Text("role"), Dz1Text("Gitsn_BasicVehicleRole"), 0 },
	{ Dz1Text("subrole"), Dz1Text("Gitsn_RequestSubRole"), 1 },
	{ Dz1Text("request"), Dz1Text("Gitsn_RequestImportanceLevel"), 2 },
	{ Dz1Text("iso3883"), Dz1Text("s64_t"), 3 },
	{ Dz1Text("hpmsType"), Dz1Text("Gitsn_VehicleType"), 4 },
	{ NULL, NULL, -1 }
};
// Gitsn_RequestorType
////////////////////////////////////////////////////////////////////////////////
...
static bool_t Gitsn_RequestorPositionVector_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	case 1: p->heading =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 2: Gitsn_TransmissionAndSpeed_delAndSetNull(&p->speed); p->speed = (Gitsn_TransmissionAndSpeed *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_RequestorPositionVector_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_RequestorPositionVector *p = (Gitsn_RequestorPositionVector *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	...
	case 1: return &p->heading;
	case 2: return p->speed;
	default: return NULL;
	}
}

static Dz1JsonSpecChild Gitsn_RequestorPositionVector_membs[] =
{
	{ Dz1Text("position"), Dz1Text("Gitsn_Position3D"), 0 },
	{ Dz1Text("heading"), Dz1Text("s64_t"), 1 },
	{ Dz1Text("speed"), Dz1Text("Gitsn_TransmissionAndSpeed"), 2 },
	{ NULL, NULL, -1 }
};
// Gitsn_RequestorPositionVector
////////////////////////////////////////////////////////////////////////////////
...
static bool_t Gitsn_TransitVehicleOccupancy_load(struct Dz1JsonVarBind *dst_obj, Dz1JsonToken *tok, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	Gitsn_TransitVehicleOccupancy v = Gitsn_TransitVehicleOccupancy_max;
	switch(tok->t)
	{
	case Dz1JsonTokenType_number:
		v = (Gitsn_TransitVehicleOccupancy)Dz1StrTo32(tok->v);
		break;
	case Dz1JsonTokenType_string:
		if ((v = Gitsn_TransitVehicleOccupancyFromStr(tok->v)) == Gitsn_TransitVehicleOccupancy_max) ERR_SET_OUT_RET(errp, EPERM, FALSE);
		break;
	default: ERR_SET_RET(errp, EPERM, FALSE);
	}

	if ((dst_obj->data = Gitsn_TransitVehicleOccupancy_new(&v, errp)) == NULL) ERR_OUT(errp);
	else Dz1Error_set(errp, 0);

	return errp->code == 0 ? TRUE : FALSE;
}

static bool_t Gitsn_TransitVehicleOccupancy_stringify(void *obj, Dz1ElasticBuf *buf, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	if (obj == NULL)
	{
	...
	{
		Gitsn_TransitVehicleOccupancy v = *(Gitsn_TransitVehicleOccupancy *)obj;
		Dz1Str str_v = Gitsn_TransitVehicleOccupancyStr(v);
		if (str_v == NULL)
		{
			if (Dz1JsonUtil_stringify_u32(buf, (u32_t)v, errp) == FALSE) ERR_OUT(errp);
			else Dz1Error_set(errp, 0);
...
static bool_t Gitsn_RequestorDescription_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	switch(id->index)
	{
	case 0: Gitsn_VehicleID_delAndSetNull(&p->id); p->id = (Gitsn_VehicleID *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 1: Gitsn_RequestorType_delAndSetNull(&p->type); p->type = (Gitsn_RequestorType *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 2: Gitsn_RequestorPositionVector_delAndSetNull(&p->position); p->position = (Gitsn_RequestorPositionVector *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 3: Dz1Str_delAndSetNull(&p->name); p->name = (Dz1Str)info->data; info->data = NULL; break; // type_scalar = 0, ptr_instance = 0
	case 4: Dz1Str_delAndSetNull(&p->routeName); p->routeName = (Dz1Str)info->data; info->data = NULL; break; // type_scalar = 0, ptr_instance = 0
	case 5: u8_t_delAndSetNull(&p->transitStatus); p->transitStatus = (u8_t *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 6: p->transitOccupancy =  *(Gitsn_TransitVehicleOccupancy *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 7: p->transitSchedule =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_RequestorDescription_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_RequestorDescription *p = (Gitsn_RequestorDescription *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	...
	case 4: return p->routeName;
	case 5: return p->transitStatus;
	case 6: return &p->transitOccupancy;
	case 7: return &p->transitSchedule;
	default: return NULL;
	}
}

static Dz1JsonSpecChild Gitsn_RequestorDescription_membs[] =
{
	{ Dz1Text("id"), Dz1Text("Gitsn_VehicleID"), 0 },
	{ Dz1Text("type"), Dz1Text("Gitsn_RequestorType"), 1 },
	{ Dz1Text("position"), Dz1Text("Gitsn_RequestorPositionVector"), 2 },
	{ Dz1Text("name"), Dz1Text("Dz1Str"), 3 },
	{ Dz1Text("routeName"), Dz1Text("Dz1Str"), 4 },
	{ Dz1Text("transitStatus"), Dz1Text("u8_t"), 5 },
	{ Dz1Text("transitOccupancy"), Dz1Text("Gitsn_TransitVehicleOccupancy"), 6 },
	{ Dz1Text("transitSchedule"), Dz1Text("s64_t"), 7 },
	{ NULL, NULL, -1 }
};
// Gitsn_RequestorDescription
////////////////////////////////////////////////////////////////////////////////
...
static bool_t Gitsn_SignalRequestMessage_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	{
	case 0: p->timeStamp =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 1: p->second =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 2: p->sequenceNumber =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 3: Gitsn_SignalRequestList_delAndSetNull(&p->requests); p->requests = (Gitsn_SignalRequestList *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 4: Gitsn_RequestorDescription_delAndSetNull(&p->requestor); p->requestor = (Gitsn_RequestorDescription *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_SignalRequestMessage_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_SignalRequestMessage *p = (Gitsn_SignalRequestMessage *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	...
	case 2: return &p->sequenceNumber;
	case 3: return p->requests;
	case 4: return p->requestor;
	default: return NULL;
	}
}

static Dz1JsonSpecChild Gitsn_SignalRequestMessage_membs[] =
{
	{ Dz1Text("timeStamp"), Dz1Text("s64_t"), 0 },
	{ Dz1Text("second"), Dz1Text("s64_t"), 1 },
	{ Dz1Text("sequenceNumber"), Dz1Text("s64_t"), 2 },
	{ Dz1Text("requests"), Dz1Text("Gitsn_SignalRequestList"), 3 },
	{ Dz1Text("requestor"), Dz1Text("Gitsn_RequestorDescription"), 4 },
	{ NULL, NULL, -1 }
};
// Gitsn_SignalRequestMessage
////////////////////////////////////////////////////////////////////////////////
...
static bool_t Gitsn_SignalRequesterInfo_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	{
	case 0: Gitsn_VehicleID_delAndSetNull(&p->id); p->id = (Gitsn_VehicleID *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 1: p->request =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 2: p->sequenceNumber =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 3: p->role =  *(Gitsn_BasicVehicleRole *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 4: Gitsn_RequestorType_delAndSetNull(&p->typeData); p->typeData = (Gitsn_RequestorType *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_SignalRequesterInfo_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_SignalRequesterInfo *p = (Gitsn_SignalRequesterInfo *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	...
	case 2: return &p->sequenceNumber;
	case 3: return &p->role;
	case 4: return p->typeData;
	default: return NULL;
	}
}

static Dz1JsonSpecChild Gitsn_SignalRequesterInfo_membs[] =
{
	{ Dz1Text("id"), Dz1Text("Gitsn_VehicleID"), 0 },
	{ Dz1Text("request"), Dz1Text("s64_t"), 1 },
	{ Dz1Text("sequenceNumber"), Dz1Text("s64_t"), 2 },
	{ Dz1Text("role"), Dz1Text("Gitsn_BasicVehicleRole"), 3 },
	{ Dz1Text("typeData"), Dz1Text("Gitsn_RequestorType"), 4 },
	{ NULL, NULL, -1 }
};
// Gitsn_SignalRequesterInfo
////////////////////////////////////////////////////////////////////////////////
...
static bool_t Gitsn_PrioritizationResponseStatus_load(struct Dz1JsonVarBind *dst_obj, Dz1JsonToken *tok, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	Gitsn_PrioritizationResponseStatus v = Gitsn_PrioritizationResponseStatus_max;
	switch(tok->t)
	{
	case Dz1JsonTokenType_number:
		v = (Gitsn_PrioritizationResponseStatus)Dz1StrTo32(tok->v);
		break;
	case Dz1JsonTokenType_string:
		if ((v = Gitsn_PrioritizationResponseStatusFromStr(tok->v)) == Gitsn_PrioritizationResponseStatus_max) ERR_SET_OUT_RET(errp, EPERM, FALSE);
		break;
	default: ERR_SET_RET(errp, EPERM, FALSE);
	}

	if ((dst_obj->data = Gitsn_PrioritizationResponseStatus_new(&v, errp)) == NULL) ERR_OUT(errp);
	else Dz1Error_set(errp, 0);

	return errp->code == 0 ? TRUE : FALSE;
}

static bool_t Gitsn_PrioritizationResponseStatus_stringify(void *obj, Dz1ElasticBuf *buf, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	if (obj == NULL)
	{
	...
	{
		Gitsn_PrioritizationResponseStatus v = *(Gitsn_PrioritizationResponseStatus *)obj;
		Dz1Str str_v = Gitsn_PrioritizationResponseStatusStr(v);
		if (str_v == NULL)
		{
			if (Dz1JsonUtil_stringify_u32(buf, (u32_t)v, errp) == FALSE) ERR_OUT(errp);
			else Dz1Error_set(errp, 0);
...
static bool_t Gitsn_SignalStatusPackage_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	switch(id->index)
	{
	case 0: Gitsn_SignalRequesterInfo_delAndSetNull(&p->requester); p->requester = (Gitsn_SignalRequesterInfo *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 1: Gitsn_IntersectionAccessPoint_delAndSetNull(&p->inboundOn); p->inboundOn = (Gitsn_IntersectionAccessPoint *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 2: Gitsn_IntersectionAccessPoint_delAndSetNull(&p->outboundOn); p->outboundOn = (Gitsn_IntersectionAccessPoint *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 3: p->minute =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 4: p->second =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 5: p->duration =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 6: p->status =  *(Gitsn_PrioritizationResponseStatus *)info->data; break; // type_scalar = 1, ptr_instance = 0
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_SignalStatusPackage_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_SignalStatusPackage *p = (Gitsn_SignalStatusPackage *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	{
	case 0: return p->requester;
	case 1: return p->inboundOn;
	case 2: return p->outboundOn;
	case 3: return &p->minute;
	case 4: return &p->second;
	case 5: return &p->duration;
	case 6: return &p->status;
	default: return NULL;
	}
}

static Dz1JsonSpecChild Gitsn_SignalStatusPackage_membs[] =
{
	{ Dz1Text("requester"), Dz1Text("Gitsn_SignalRequesterInfo"), 0 },
	{ Dz1Text("inboundOn"), Dz1Text("Gitsn_IntersectionAccessPoint"), 1 },
	{ Dz1Text("outboundOn"), Dz1Text("Gitsn_IntersectionAccessPoint"), 2 },
	{ Dz1Text("minute"), Dz1Text("s64_t"), 3 },
	{ Dz1Text("second"), Dz1Text("s64_t"), 4 },
	{ Dz1Text("duration"), Dz1Text("s64_t"), 5 },
	{ Dz1Text("status"), Dz1Text("Gitsn_PrioritizationResponseStatus"), 6 },
	{ NULL, NULL, -1 }
};
// Gitsn_SignalStatusPackage
////////////////////////////////////////////////////////////////////////////////
...
static bool_t Gitsn_SignalStatusPackageList_append(void *dst_ptr, Dz1JsonVarBind *var, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	Gitsn_SignalStatusPackageList *dst = (Gitsn_SignalStatusPackageList *)dst_ptr;
	Gitsn_SignalStatusPackage *node = NULL;
	if (dst == NULL || var == NULL || (node = (Gitsn_SignalStatusPackage *)var->data) == NULL) ERR_SET_OUT(errp, EINVAL);
	else if ((*errp = dst->add(dst, node)).code) ERR_OUT(errp);
	else
	{
		var->data = NULL;
...
static u32_t Gitsn_SignalStatusPackageList_cnt(void *ptr)
{
	u32_t ret = 0;
	Gitsn_SignalStatusPackageList *list = (Gitsn_SignalStatusPackageList *)ptr;
	if (list != NULL) ret = list->count(list);
	return ret;
}

static Dz1Error Gitsn_SignalStatusPackageList_tlv(void *obj, Dz1JsonArrayEntryF f, void *arg)
{
	DZ1_ERROR_SAFE_VAR(errp, err);
	Gitsn_SignalStatusPackageList *list = (Gitsn_SignalStatusPackageList *)obj;
	if (list == NULL) ERR_SET_OUT(errp, EINVAL);
	else if ((*errp = list->travel(list, f, arg)).code) ERR_OUT(errp);
	else Dz1Error_set(errp, 0);
	return err;
...
////////////////////////////////////////////////////////////////////////////////
// Gitsn_SignalStatus
static bool_t Gitsn_SignalStatus_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	switch(id->index)
	{
	case 0: p->sequenceNumber =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 1: Gitsn_IntersectionReferenceID_delAndSetNull(&p->id); p->id = (Gitsn_IntersectionReferenceID *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 2: Gitsn_SignalStatusPackageList_delAndSetNull(&p->sigStatus); p->sigStatus = (Gitsn_SignalStatusPackageList *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_SignalStatus_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_SignalStatus *p = (Gitsn_SignalStatus *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	{
	case 0: return &p->sequenceNumber;
	case 1: return p->id;
	case 2: return p->sigStatus;
	default: return NULL;
	}
}

static Dz1JsonSpecChild Gitsn_SignalStatus_membs[] =
{
	{ Dz1Text("sequenceNumber"), Dz1Text("s64_t"), 0 },
	{ Dz1Text("id"), Dz1Text("Gitsn_IntersectionReferenceID"), 1 },
	{ Dz1Text("sigStatus"), Dz1Text("Gitsn_SignalStatusPackageList"), 2 },
	{ NULL, NULL, -1 }
};
// Gitsn_SignalStatus
////////////////////////////////////////////////////////////////////////////////
...
static bool_t Gitsn_SignalStatusList_append(void *dst_ptr, Dz1JsonVarBind *var, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	Gitsn_SignalStatusList *dst = (Gitsn_SignalStatusList *)dst_ptr;
	Gitsn_SignalStatus *node = NULL;
	if (dst == NULL || var == NULL || (node = (Gitsn_SignalStatus *)var->data) == NULL) ERR_SET_OUT(errp, EINVAL);
	else if ((*errp = dst->add(dst, node)).code) ERR_OUT(errp);
	else
	{
		var->data = NULL;
...
static u32_t Gitsn_SignalStatusList_cnt(void *ptr)
{
	u32_t ret = 0;
	Gitsn_SignalStatusList *list = (Gitsn_SignalStatusList *)ptr;
	if (list != NULL) ret = list->count(list);
	return ret;
}

static Dz1Error Gitsn_SignalStatusList_tlv(void *obj, Dz1JsonArrayEntryF f, void *arg)
{
	DZ1_ERROR_SAFE_VAR(errp, err);
	Gitsn_SignalStatusList *list = (Gitsn_SignalStatusList *)obj;
	if (list == NULL) ERR_SET_OUT(errp, EINVAL);
	else if ((*errp = list->travel(list, f, arg)).code) ERR_OUT(errp);
	else Dz1Error_set(errp, 0);
	return err;
...
static bool_t Gitsn_SignalStatusMessage_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	case 1: p->second =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 2: p->sequenceNumber =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 3: Gitsn_SignalStatusList_delAndSetNull(&p->status); p->status = (Gitsn_SignalStatusList *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_SignalStatusMessage_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_SignalStatusMessage *p = (Gitsn_SignalStatusMessage *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
...
static Dz1JsonSpecChild Gitsn_SignalStatusMessage_membs[] =
{
	{ Dz1Text("timeStamp"), Dz1Text("s64_t"), 0 },
	{ Dz1Text("second"), Dz1Text("s64_t"), 1 },
	{ Dz1Text("sequenceNumber"), Dz1Text("s64_t"), 2 },
	{ Dz1Text("status"), Dz1Text("Gitsn_SignalStatusList"), 3 },
	{ NULL, NULL, -1 }
};
// Gitsn_SignalStatusMessage
////////////////////////////////////////////////////////////////////////////////
...
static bool_t Gitsn_NumberOfParticipantsInCluster_load(struct Dz1JsonVarBind *dst_obj, Dz1JsonToken *tok, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	Gitsn_NumberOfParticipantsInCluster v = Gitsn_NumberOfParticipantsInCluster_max;
	switch(tok->t)
	{
	case Dz1JsonTokenType_number:
		v = (Gitsn_NumberOfParticipantsInCluster)Dz1StrTo32(tok->v);
		break;
	case Dz1JsonTokenType_string:
		if ((v = Gitsn_NumberOfParticipantsInClusterFromStr(tok->v)) == Gitsn_NumberOfParticipantsInCluster_max) ERR_SET_OUT_RET(errp, EPERM, FALSE);
		break;
	default: ERR_SET_RET(errp, EPERM, FALSE);
	}

	if ((dst_obj->data = Gitsn_NumberOfParticipantsInCluster_new(&v, errp)) == NULL) ERR_OUT(errp);
	else Dz1Error_set(errp, 0);

	return errp->code == 0 ? TRUE : FALSE;
}

static bool_t Gitsn_NumberOfParticipantsInCluster_stringify(void *obj, Dz1ElasticBuf *buf, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	if (obj == NULL)
	{
	...
	{
		Gitsn_NumberOfParticipantsInCluster v = *(Gitsn_NumberOfParticipantsInCluster *)obj;
		Dz1Str str_v = Gitsn_NumberOfParticipantsInClusterStr(v);
		if (str_v == NULL)
		{
			if (Dz1JsonUtil_stringify_u32(buf, (u32_t)v, errp) == FALSE) ERR_OUT(errp);
			else Dz1Error_set(errp, 0);
...
static bool_t Gitsn_PublicSafetyEventResponderWorkerType_load(struct Dz1JsonVarBind *dst_obj, Dz1JsonToken *tok, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	Gitsn_PublicSafetyEventResponderWorkerType v = Gitsn_PublicSafetyEventResponderWorkerType_max;
	switch(tok->t)
	{
	case Dz1JsonTokenType_number:
		v = (Gitsn_PublicSafetyEventResponderWorkerType)Dz1StrTo32(tok->v);
		break;
	case Dz1JsonTokenType_string:
		if ((v = Gitsn_PublicSafetyEventResponderWorkerTypeFromStr(tok->v)) == Gitsn_PublicSafetyEventResponderWorkerType_max) ERR_SET_OUT_RET(errp, EPERM, FALSE);
		break;
	default: ERR_SET_RET(errp, EPERM, FALSE);
	}

	if ((dst_obj->data = Gitsn_PublicSafetyEventResponderWorkerType_new(&v, errp)) == NULL) ERR_OUT(errp);
	else Dz1Error_set(errp, 0);

	return errp->code == 0 ? TRUE : FALSE;
}

static bool_t Gitsn_PublicSafetyEventResponderWorkerType_stringify(void *obj, Dz1ElasticBuf *buf, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	if (obj == NULL)
	{
	...
	{
		Gitsn_PublicSafetyEventResponderWorkerType v = *(Gitsn_PublicSafetyEventResponderWorkerType *)obj;
		Dz1Str str_v = Gitsn_PublicSafetyEventResponderWorkerTypeStr(v);
		if (str_v == NULL)
		{
			if (Dz1JsonUtil_stringify_u32(buf, (u32_t)v, errp) == FALSE) ERR_OUT(errp);
			else Dz1Error_set(errp, 0);
...
////////////////////////////////////////////////////////////////////////////////
// Gitsn_AnimalType
static bool_t Gitsn_AnimalType_load(struct Dz1JsonVarBind *dst_obj, Dz1JsonToken *tok, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	Gitsn_AnimalType v = Gitsn_AnimalType_max;
	switch(tok->t)
	{
	case Dz1JsonTokenType_number:
		v = (Gitsn_AnimalType)Dz1StrTo32(tok->v);
		break;
	case Dz1JsonTokenType_string:
		if ((v = Gitsn_AnimalTypeFromStr(tok->v)) == Gitsn_AnimalType_max) ERR_SET_OUT_RET(errp, EPERM, FALSE);
		break;
	default: ERR_SET_RET(errp, EPERM, FALSE);
	}

	if ((dst_obj->data = Gitsn_AnimalType_new(&v, errp)) == NULL) ERR_OUT(errp);
	else Dz1Error_set(errp, 0);

	return errp->code == 0 ? TRUE : FALSE;
}

static bool_t Gitsn_AnimalType_stringify(void *obj, Dz1ElasticBuf *buf, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	if (obj == NULL)
	{
	...
	{
		Gitsn_AnimalType v = *(Gitsn_AnimalType *)obj;
		Dz1Str str_v = Gitsn_AnimalTypeStr(v);
		if (str_v == NULL)
		{
			if (Dz1JsonUtil_stringify_u32(buf, (u32_t)v, errp) == FALSE) ERR_OUT(errp);
			else Dz1Error_set(errp, 0);
...
static bool_t Gitsn_PersonalSafetyMessage_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	case 1: p->secMark =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 2: p->msgCnt =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 3: Dz1Str_delAndSetNull(&p->id); p->id = (Dz1Str)info->data; info->data = NULL; break; // type_scalar = 0, ptr_instance = 0
	case 4: Gitsn_Position3D_delAndSetNull(&p->position); p->position = (Gitsn_Position3D *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 5: Gitsn_PositionalAccuracy_delAndSetNull(&p->accuracy); p->accuracy = (Gitsn_PositionalAccuracy *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 6: p->speed =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 7: p->heading =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 8: Gitsn_AccelerationSet4Way_delAndSetNull(&p->accelSet); p->accelSet = (Gitsn_AccelerationSet4Way *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 9: Gitsn_PathHistory_delAndSetNull(&p->pathHistory); p->pathHistory = (Gitsn_PathHistory *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 10: Gitsn_PathPrediction_delAndSetNull(&p->pathPrediction); p->pathPrediction = (Gitsn_PathPrediction *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 11: Gitsn_PropelledInformation_delAndSetNull(&p->propulsion); p->propulsion = (Gitsn_PropelledInformation *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 12: u8_t_delAndSetNull(&p->useState); p->useState = (u8_t *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 13: p->crossRequest =  *(bool_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 14: p->crossState =  *(bool_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 15: p->clusterSize =  *(Gitsn_NumberOfParticipantsInCluster *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 16: p->clusterRadius =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 17: p->eventResponderType =  *(Gitsn_PublicSafetyEventResponderWorkerType *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 18: u8_t_delAndSetNull(&p->activityType); p->activityType = (u8_t *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 19: u8_t_delAndSetNull(&p->activitySubType); p->activitySubType = (u8_t *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 20: u8_t_delAndSetNull(&p->assistType); p->assistType = (u8_t *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 21: u8_t_delAndSetNull(&p->sizing); p->sizing = (u8_t *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 22: p->attachment =  *(Gitsn_Attachment *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 23: p->attachmentRadius =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 24: p->animalType =  *(Gitsn_AnimalType *)info->data; break; // type_scalar = 1, ptr_instance = 0
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *Gitsn_PersonalSafetyMessage_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_PersonalSafetyMessage *p = (Gitsn_PersonalSafetyMessage *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	...
	case 1: return &p->secMark;
	case 2: return &p->msgCnt;
	case 3: return p->id;
	case 4: return p->position;
	case 5: return p->accuracy;
	case 6: return &p->speed;
	case 7: return &p->heading;
	case 8: return p->accelSet;
	case 9: return p->pathHistory;
	case 10: return p->pathPrediction;
	case 11: return p->propulsion;
	case 12: return p->useState;
	case 13: return &p->crossRequest;
	case 14: return &p->crossState;
	case 15: return &p->clusterSize;
	case 16: return &p->clusterRadius;
	case 17: return &p->eventResponderType;
	case 18: return p->activityType;
	case 19: return p->activitySubType;
	case 20: return p->assistType;
	case 21: return p->sizing;
	case 22: return &p->attachment;
	case 23: return &p->attachmentRadius;
	case 24: return &p->animalType;
	default: return NULL;
	}
}

static Dz1JsonSpecChild Gitsn_PersonalSafetyMessage_membs[] =
{
	{ Dz1Text("basicType"), Dz1Text("Gitsn_PersonalDeviceUserType"), 0 },
	{ Dz1Text("secMark"), Dz1Text("s64_t"), 1 },
	{ Dz1Text("msgCnt"), Dz1Text("s64_t"), 2 },
	{ Dz1Text("id"), Dz1Text("Dz1Str"), 3 },
	{ Dz1Text("position"), Dz1Text("Gitsn_Position3D"), 4 },
	{ Dz1Text("accuracy"), Dz1Text("Gitsn_PositionalAccuracy"), 5 },
	{ Dz1Text("speed"), Dz1Text("s64_t"), 6 },
	{ Dz1Text("heading"), Dz1Text("s64_t"), 7 },
	{ Dz1Text("accelSet"), Dz1Text("Gitsn_AccelerationSet4Way"), 8 },
	{ Dz1Text("pathHistory"), Dz1Text("Gitsn_PathHistory"), 9 },
	{ Dz1Text("pathPrediction"), Dz1Text("Gitsn_PathPrediction"), 10 },
	{ Dz1Text("propulsion"), Dz1Text("Gitsn_PropelledInformation"), 11 },
	{ Dz1Text("useState"), Dz1Text("u8_t"), 12 },
	{ Dz1Text("crossRequest"), Dz1Text("bool_t"), 13 },
	{ Dz1Text("crossState"), Dz1Text("bool_t"), 14 },
	{ Dz1Text("clusterSize"), Dz1Text("Gitsn_NumberOfParticipantsInCluster"), 15 },
	{ Dz1Text("clusterRadius"), Dz1Text("s64_t"), 16 },
	{ Dz1Text("eventResponderType"), Dz1Text("Gitsn_PublicSafetyEventResponderWorkerType"), 17 },
	{ Dz1Text("activityType"), Dz1Text("u8_t"), 18 },
	{ Dz1Text("activitySubType"), Dz1Text("u8_t"), 19 },
	{ Dz1Text("assistType"), Dz1Text("u8_t"), 20 },
	{ Dz1Text("sizing"), Dz1Text("u8_t"), 21 },
	{ Dz1Text("attachment"), Dz1Text("Gitsn_Attachment"), 22 },
	{ Dz1Text("attachmentRadius"), Dz1Text("s64_t"), 23 },
	{ Dz1Text("animalType"), Dz1Text("Gitsn_AnimalType"), 24 },
	{ NULL, NULL, -1 }
};
// Gitsn_PersonalSafetyMessage
////////////////////////////////////////////////////////////////////////////////
...
static bool_t Gitsn_TrafficLightStatusMessage_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	switch(id->index)
	{
	case 0: p->trafficLightID =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 1: p->lat =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 2: p->lon =  *(s64_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 3: u8_t_delAndSetNull(&p->operationStatus); p->operationStatus = (u8_t *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
	case 4: u8_t_delAndSetNull(&p->controllerStatus); p->controllerStatus = (u8_t *)info->data; info->data = NULL; break; // type_scalar = 1, ptr_instance = 1
...
static void *Gitsn_TrafficLightStatusMessage_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	Gitsn_TrafficLightStatusMessage *p = (Gitsn_TrafficLightStatusMessage *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
...
static Dz1JsonSpecChild Gitsn_TrafficLightStatusMessage_membs[] =
{
	{ Dz1Text("trafficLightID"), Dz1Text("s64_t"), 0 },
	{ Dz1Text("lat"), Dz1Text("s64_t"), 1 },
	{ Dz1Text("lon"), Dz1Text("s64_t"), 2 },
	{ Dz1Text("operationStatus"), Dz1Text("u8_t"), 3 },
	{ Dz1Text("controllerStatus"), Dz1Text("u8_t"), 4 },
...
////////////////////////////////////////////////////////////////////////////////
// GitsnDSRCMsg
static bool_t GitsnDSRCMsg_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	switch(id->index)
	{
	case GitsnDSRCMsgPresent_map: Gitsn_MapData_delAndSetNull(&p->x.map); p->x.map =(Gitsn_MapData *)info->data; info->data = NULL; break;
	case GitsnDSRCMsgPresent_spat: Gitsn_SPAT_delAndSetNull(&p->x.spat); p->x.spat =(Gitsn_SPAT *)info->data; info->data = NULL; break;
	case GitsnDSRCMsgPresent_bsm: Gitsn_BasicSafetyMessage_delAndSetNull(&p->x.bsm); p->x.bsm =(Gitsn_BasicSafetyMessage *)info->data; info->data = NULL; break;
	case GitsnDSRCMsgPresent_eva: Gitsn_EmergencyVehicleAlert_delAndSetNull(&p->x.eva); p->x.eva =(Gitsn_EmergencyVehicleAlert *)info->data; info->data = NULL; break;
	case GitsnDSRCMsgPresent_pvd: Gitsn_ProbeVehicleData_delAndSetNull(&p->x.pvd); p->x.pvd =(Gitsn_ProbeVehicleData *)info->data; info->data = NULL; break;
	case GitsnDSRCMsgPresent_rsa: Gitsn_RoadSideAlert_delAndSetNull(&p->x.rsa); p->x.rsa =(Gitsn_RoadSideAlert *)info->data; info->data = NULL; break;
	case GitsnDSRCMsgPresent_rtcm: Gitsn_RTCMcorrections_delAndSetNull(&p->x.rtcm); p->x.rtcm =(Gitsn_RTCMcorrections *)info->data; info->data = NULL; break;
	case GitsnDSRCMsgPresent_srm: Gitsn_SignalRequestMessage_delAndSetNull(&p->x.srm); p->x.srm =(Gitsn_SignalRequestMessage *)info->data; info->data = NULL; break;
	case GitsnDSRCMsgPresent_ssm: Gitsn_SignalStatusMessage_delAndSetNull(&p->x.ssm); p->x.ssm =(Gitsn_SignalStatusMessage *)info->data; info->data = NULL; break;
	case GitsnDSRCMsgPresent_tim: Gitsn_TravelerInformation_delAndSetNull(&p->x.tim); p->x.tim =(Gitsn_TravelerInformation *)info->data; info->data = NULL; break;
	case GitsnDSRCMsgPresent_psm: Gitsn_PersonalSafetyMessage_delAndSetNull(&p->x.psm); p->x.psm =(Gitsn_PersonalSafetyMessage *)info->data; info->data = NULL; break;
	case GitsnDSRCMsgPresent_sdsm: Gitsn_SensorDataSharingMessage_delAndSetNull(&p->x.sdsm); p->x.sdsm =(Gitsn_SensorDataSharingMessage *)info->data; info->data = NULL; break;
	case GitsnDSRCMsgPresent_tls: Gitsn_TLS_delAndSetNull(&p->x.tls); p->x.tls =(Gitsn_TLS *)info->data; info->data = NULL; break;
	case GitsnDSRCMsgPresent_tlsm: Gitsn_TrafficLightStatusMessage_delAndSetNull(&p->x.tlsm); p->x.tlsm =(Gitsn_TrafficLightStatusMessage *)info->data; info->data = NULL; break;
	default: ERR_SET_OUT_RET(errp, ENOENT, FALSE);
	}
	p->present = (GitsnDSRCMsgPresent)id->index;
	return errp->code == 0 ? TRUE : FALSE;
}

static void *GitsnDSRCMsg_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	GitsnDSRCMsg *p = (GitsnDSRCMsg *)ptr;
	Dz1JsonSpecChild *id = (*info) = Dz1JsonSpecChilds_findByIdx(lists, p->present);
	if (id != NULL)
	{
		switch(p->present)
		{
		case GitsnDSRCMsgPresent_map: return p->x.map;
		case GitsnDSRCMsgPresent_spat: return p->x.spat;
		case GitsnDSRCMsgPresent_bsm: return p->x.bsm;
		case GitsnDSRCMsgPresent_eva: return p->x.eva;
		case GitsnDSRCMsgPresent_pvd: return p->x.pvd;
		case GitsnDSRCMsgPresent_rsa: return p->x.rsa;
		case GitsnDSRCMsgPresent_rtcm: return p->x.rtcm;
		case GitsnDSRCMsgPresent_srm: return p->x.srm;
		case GitsnDSRCMsgPresent_ssm: return p->x.ssm;
		case GitsnDSRCMsgPresent_tim: return p->x.tim;
		case GitsnDSRCMsgPresent_psm: return p->x.psm;
		case GitsnDSRCMsgPresent_sdsm: return p->x.sdsm;
		case GitsnDSRCMsgPresent_tls: return p->x.tls;
		case GitsnDSRCMsgPresent_tlsm: return p->x.tlsm;
		default: return NULL;
		}
	}
	else return NULL;
...
static Dz1JsonSpecChild GitsnDSRCMsg_membs[] =
{
	{ Dz1Text("map"), Dz1Text("Gitsn_MapData"), GitsnDSRCMsgPresent_map },
	{ Dz1Text("spat"), Dz1Text("Gitsn_SPAT"), GitsnDSRCMsgPresent_spat },
	{ Dz1Text("bsm"), Dz1Text("Gitsn_BasicSafetyMessage"), GitsnDSRCMsgPresent_bsm },
	{ Dz1Text("eva"), Dz1Text("Gitsn_EmergencyVehicleAlert"), GitsnDSRCMsgPresent_eva },
	{ Dz1Text("pvd"), Dz1Text("Gitsn_ProbeVehicleData"), GitsnDSRCMsgPresent_pvd },
	{ Dz1Text("rsa"), Dz1Text("Gitsn_RoadSideAlert"), GitsnDSRCMsgPresent_rsa },
	{ Dz1Text("rtcm"), Dz1Text("Gitsn_RTCMcorrections"), GitsnDSRCMsgPresent_rtcm },
	{ Dz1Text("srm"), Dz1Text("Gitsn_SignalRequestMessage"), GitsnDSRCMsgPresent_srm },
	{ Dz1Text("ssm"), Dz1Text("Gitsn_SignalStatusMessage"), GitsnDSRCMsgPresent_ssm },
	{ Dz1Text("tim"), Dz1Text("Gitsn_TravelerInformation"), GitsnDSRCMsgPresent_tim },
	{ Dz1Text("psm"), Dz1Text("Gitsn_PersonalSafetyMessage"), GitsnDSRCMsgPresent_psm },
	{ Dz1Text("sdsm"), Dz1Text("Gitsn_SensorDataSharingMessage"), GitsnDSRCMsgPresent_sdsm },
	{ Dz1Text("tls"), Dz1Text("Gitsn_TLS"), GitsnDSRCMsgPresent_tls },
	{ Dz1Text("tlsm"), Dz1Text("Gitsn_TrafficLightStatusMessage"), GitsnDSRCMsgPresent_tlsm },
	{ NULL, NULL, -1 }
};
// GitsnDSRCMsg
////////////////////////////////////////////////////////////////////////////////
...
static bool_t GitsnDSRCMsgTestConfiguration_setChild(void *ptr, Dz1JsonVarBind *info, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	case 1: p->test_map =  *(bool_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 2: p->test_spat =  *(bool_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 3: p->test_bsm =  *(bool_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 4: p->test_eva =  *(bool_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 5: p->test_pvd =  *(bool_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 6: p->test_rsa =  *(bool_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 7: p->test_rtcm =  *(bool_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 8: p->test_srm =  *(bool_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 9: p->test_ssm =  *(bool_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 10: p->test_tim =  *(bool_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 11: p->test_psm =  *(bool_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 12: p->test_sdsm =  *(bool_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	case 13: p->test_tls =  *(bool_t *)info->data; break; // type_scalar = 1, ptr_instance = 0
	default: ERR_SET_OUT(errp, ENOENT); break;
	}
	return errp->code == 0 ? TRUE : FALSE;
}

static void *GitsnDSRCMsgTestConfiguration_getChild(void *ptr, Dz1JsonSpecChild **info, Dz1JsonSpecChilds *lists)
{
	GitsnDSRCMsgTestConfiguration *p = (GitsnDSRCMsgTestConfiguration *)ptr;
	Dz1JsonSpecChild *id = *(Dz1JsonSpecChild **)info;
	switch(id->index)
	...
	case 1: return &p->test_map;
	case 2: return &p->test_spat;
	case 3: return &p->test_bsm;
	case 4: return &p->test_eva;
	case 5: return &p->test_pvd;
	case 6: return &p->test_rsa;
	case 7: return &p->test_rtcm;
	case 8: return &p->test_srm;
	case 9: return &p->test_ssm;
	case 10: return &p->test_tim;
	case 11: return &p->test_psm;
	case 12: return &p->test_sdsm;
	case 13: return &p->test_tls;
	default: return NULL;
	}
}

static Dz1JsonSpecChild GitsnDSRCMsgTestConfiguration_membs[] =
{
	{ Dz1Text("local_port"), Dz1Text("s64_t"), 0 },
	{ Dz1Text("test_map"), Dz1Text("bool_t"), 1 },
	{ Dz1Text("test_spat"), Dz1Text("bool_t"), 2 },
	{ Dz1Text("test_bsm"), Dz1Text("bool_t"), 3 },
	{ Dz1Text("test_eva"), Dz1Text("bool_t"), 4 },
	{ Dz1Text("test_pvd"), Dz1Text("bool_t"), 5 },
	{ Dz1Text("test_rsa"), Dz1Text("bool_t"), 6 },
	{ Dz1Text("test_rtcm"), Dz1Text("bool_t"), 7 },
	{ Dz1Text("test_srm"), Dz1Text("bool_t"), 8 },
	{ Dz1Text("test_ssm"), Dz1Text("bool_t"), 9 },
	{ Dz1Text("test_tim"), Dz1Text("bool_t"), 10 },
	{ Dz1Text("test_psm"), Dz1Text("bool_t"), 11 },
	{ Dz1Text("test_sdsm"), Dz1Text("bool_t"), 12 },
	{ Dz1Text("test_tls"), Dz1Text("bool_t"), 13 },
	{ NULL, NULL, -1 }
};
// GitsnDSRCMsgTestConfiguration
////////////////////////////////////////////////////////////////////////////////
...
bool_t GitsnDSRCMsgTestScenarioDef_initJsonSpec(Dz1JsonSpec *dst, Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	else
	{
		...
		else if ((*errp = Dz1JsonSpec_add_primitive(dst, Dz1Text("Gitsn_ResponseType"), Gitsn_ResponseType_gen, Gitsn_ResponseType_del, Gitsn_ResponseType_load, Gitsn_ResponseType_stringify)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JsonSpec_add_primitive(dst, Dz1Text("Gitsn_SirenInUse"), Gitsn_SirenInUse_gen, Gitsn_SirenInUse_del, Gitsn_SirenInUse_load, Gitsn_SirenInUse_stringify)).code) ERR_OUT(errp);
		...
		else if ((*errp = Dz1JsonSpec_add_primitive(dst, Dz1Text("Gitsn_MultiVehicleResponse"), Gitsn_MultiVehicleResponse_gen, Gitsn_MultiVehicleResponse_del, Gitsn_MultiVehicleResponse_load, Gitsn_MultiVehicleResponse_stringify)).code) ERR_OUT(errp);
		...
		else if ((*errp = Dz1JsonSpec_add_primitive(dst, Dz1Text("Gitsn_RTCM_Revision"), Gitsn_RTCM_Revision_gen, Gitsn_RTCM_Revision_del, Gitsn_RTCM_Revision_load, Gitsn_RTCM_Revision_stringify)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JsonSpec_add_primitive(dst, Dz1Text("Gitsn_PriorityRequestType"), Gitsn_PriorityRequestType_gen, Gitsn_PriorityRequestType_del, Gitsn_PriorityRequestType_load, Gitsn_PriorityRequestType_stringify)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JsonSpec_add_primitive(dst, Dz1Text("Gitsn_RequestSubRole"), Gitsn_RequestSubRole_gen, Gitsn_RequestSubRole_del, Gitsn_RequestSubRole_load, Gitsn_RequestSubRole_stringify)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JsonSpec_add_primitive(dst, Dz1Text("Gitsn_RequestImportanceLevel"), Gitsn_RequestImportanceLevel_gen, Gitsn_RequestImportanceLevel_del, Gitsn_RequestImportanceLevel_load, Gitsn_RequestImportanceLevel_stringify)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JsonSpec_add_primitive(dst, Dz1Text("Gitsn_TransitVehicleOccupancy"), Gitsn_TransitVehicleOccupancy_gen, Gitsn_TransitVehicleOccupancy_del, Gitsn_TransitVehicleOccupancy_load, Gitsn_TransitVehicleOccupancy_stringify)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JsonSpec_add_primitive(dst, Dz1Text("Gitsn_PrioritizationResponseStatus"), Gitsn_PrioritizationResponseStatus_gen, Gitsn_PrioritizationResponseStatus_del, Gitsn_PrioritizationResponseStatus_load, Gitsn_PrioritizationResponseStatus_stringify)).code) ERR_OUT(errp);
		...
		else if ((*errp = Dz1JsonSpec_add_primitive(dst, Dz1Text("Gitsn_NumberOfParticipantsInCluster"), Gitsn_NumberOfParticipantsInCluster_gen, Gitsn_NumberOfParticipantsInCluster_del, Gitsn_NumberOfParticipantsInCluster_load, Gitsn_NumberOfParticipantsInCluster_stringify)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JsonSpec_add_primitive(dst, Dz1Text("Gitsn_PublicSafetyEventResponderWorkerType"), Gitsn_PublicSafetyEventResponderWorkerType_gen, Gitsn_PublicSafetyEventResponderWorkerType_del, Gitsn_PublicSafetyEventResponderWorkerType_load, Gitsn_PublicSafetyEventResponderWorkerType_stringify)).code) ERR_OUT(errp);
		...
		else if ((*errp = Dz1JsonSpec_add_primitive(dst, Dz1Text("Gitsn_AnimalType"), Gitsn_AnimalType_gen, Gitsn_AnimalType_del, Gitsn_AnimalType_load, Gitsn_AnimalType_stringify)).code) ERR_OUT(errp);
		...
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_PrivilegedEvents"), Gitsn_PrivilegedEvents_gen, Gitsn_PrivilegedEvents_del, Gitsn_PrivilegedEvents_setChild, Gitsn_PrivilegedEvents_getChild, Gitsn_PrivilegedEvents_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_EmergencyDetails"), Gitsn_EmergencyDetails_gen, Gitsn_EmergencyDetails_del, Gitsn_EmergencyDetails_setChild, Gitsn_EmergencyDetails_getChild, Gitsn_EmergencyDetails_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_EmergencyVehicleAlert"), Gitsn_EmergencyVehicleAlert_gen, Gitsn_EmergencyVehicleAlert_del, Gitsn_EmergencyVehicleAlert_setChild, Gitsn_EmergencyVehicleAlert_getChild, Gitsn_EmergencyVehicleAlert_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		...
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_AntennaOffsetSet"), Gitsn_AntennaOffsetSet_gen, Gitsn_AntennaOffsetSet_del, Gitsn_AntennaOffsetSet_setChild, Gitsn_AntennaOffsetSet_getChild, Gitsn_AntennaOffsetSet_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_RTCMheader"), Gitsn_RTCMheader_gen, Gitsn_RTCMheader_del, Gitsn_RTCMheader_setChild, Gitsn_RTCMheader_getChild, Gitsn_RTCMheader_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_RTCMcorrections"), Gitsn_RTCMcorrections_gen, Gitsn_RTCMcorrections_del, Gitsn_RTCMcorrections_setChild, Gitsn_RTCMcorrections_getChild, Gitsn_RTCMcorrections_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_IntersectionAccessPoint"), Gitsn_IntersectionAccessPoint_gen, Gitsn_IntersectionAccessPoint_del, Gitsn_IntersectionAccessPoint_setChild, Gitsn_IntersectionAccessPoint_getChild, Gitsn_IntersectionAccessPoint_membs, Dz1JsonConstructType_union)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_SignalRequest"), Gitsn_SignalRequest_gen, Gitsn_SignalRequest_del, Gitsn_SignalRequest_setChild, Gitsn_SignalRequest_getChild, Gitsn_SignalRequest_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_SignalRequestPackage"), Gitsn_SignalRequestPackage_gen, Gitsn_SignalRequestPackage_del, Gitsn_SignalRequestPackage_setChild, Gitsn_SignalRequestPackage_getChild, Gitsn_SignalRequestPackage_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_RequestorType"), Gitsn_RequestorType_gen, Gitsn_RequestorType_del, Gitsn_RequestorType_setChild, Gitsn_RequestorType_getChild, Gitsn_RequestorType_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_RequestorPositionVector"), Gitsn_RequestorPositionVector_gen, Gitsn_RequestorPositionVector_del, Gitsn_RequestorPositionVector_setChild, Gitsn_RequestorPositionVector_getChild, Gitsn_RequestorPositionVector_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_RequestorDescription"), Gitsn_RequestorDescription_gen, Gitsn_RequestorDescription_del, Gitsn_RequestorDescription_setChild, Gitsn_RequestorDescription_getChild, Gitsn_RequestorDescription_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_SignalRequestMessage"), Gitsn_SignalRequestMessage_gen, Gitsn_SignalRequestMessage_del, Gitsn_SignalRequestMessage_setChild, Gitsn_SignalRequestMessage_getChild, Gitsn_SignalRequestMessage_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_SignalRequesterInfo"), Gitsn_SignalRequesterInfo_gen, Gitsn_SignalRequesterInfo_del, Gitsn_SignalRequesterInfo_setChild, Gitsn_SignalRequesterInfo_getChild, Gitsn_SignalRequesterInfo_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_SignalStatusPackage"), Gitsn_SignalStatusPackage_gen, Gitsn_SignalStatusPackage_del, Gitsn_SignalStatusPackage_setChild, Gitsn_SignalStatusPackage_getChild, Gitsn_SignalStatusPackage_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_SignalStatus"), Gitsn_SignalStatus_gen, Gitsn_SignalStatus_del, Gitsn_SignalStatus_setChild, Gitsn_SignalStatus_getChild, Gitsn_SignalStatus_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_SignalStatusMessage"), Gitsn_SignalStatusMessage_gen, Gitsn_SignalStatusMessage_del, Gitsn_SignalStatusMessage_setChild, Gitsn_SignalStatusMessage_getChild, Gitsn_SignalStatusMessage_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		...
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_PersonalSafetyMessage"), Gitsn_PersonalSafetyMessage_gen, Gitsn_PersonalSafetyMessage_del, Gitsn_PersonalSafetyMessage_setChild, Gitsn_PersonalSafetyMessage_getChild, Gitsn_PersonalSafetyMessage_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		...
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("Gitsn_TrafficLightStatusMessage"), Gitsn_TrafficLightStatusMessage_gen, Gitsn_TrafficLightStatusMessage_del, Gitsn_TrafficLightStatusMessage_setChild, Gitsn_TrafficLightStatusMessage_getChild, Gitsn_TrafficLightStatusMessage_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("GitsnDSRCMsg"), GitsnDSRCMsg_gen, GitsnDSRCMsg_del, GitsnDSRCMsg_setChild, GitsnDSRCMsg_getChild, GitsnDSRCMsg_membs, Dz1JsonConstructType_union)).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_construct(dst, Dz1Text("GitsnDSRCMsgTestConfiguration"), GitsnDSRCMsgTestConfiguration_gen, GitsnDSRCMsgTestConfiguration_del, GitsnDSRCMsgTestConfiguration_setChild, GitsnDSRCMsgTestConfiguration_getChild, GitsnDSRCMsgTestConfiguration_membs, Dz1JsonConstructType_struct)).code) ERR_OUT(errp);
		...
		else if ((*errp = Dz1JSonSpec_add_array(dst, Dz1Text("Gitsn_RTCMmessageList"), Gitsn_RTCMmessageList_gen, Gitsn_RTCMmessageList_del, Gitsn_RTCMmessageList_append, Gitsn_RTCMmessageList_cnt, Gitsn_RTCMmessageList_tlv, Dz1Text("Dz1Str") )).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_array(dst, Dz1Text("Gitsn_SignalRequestList"), Gitsn_SignalRequestList_gen, Gitsn_SignalRequestList_del, Gitsn_SignalRequestList_append, Gitsn_SignalRequestList_cnt, Gitsn_SignalRequestList_tlv, Dz1Text("Gitsn_SignalRequestPackage") )).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_array(dst, Dz1Text("Gitsn_SignalStatusPackageList"), Gitsn_SignalStatusPackageList_gen, Gitsn_SignalStatusPackageList_del, Gitsn_SignalStatusPackageList_append, Gitsn_SignalStatusPackageList_cnt, Gitsn_SignalStatusPackageList_tlv, Dz1Text("Gitsn_SignalStatusPackage") )).code) ERR_OUT(errp);
		else if ((*errp = Dz1JSonSpec_add_array(dst, Dz1Text("Gitsn_SignalStatusList"), Gitsn_SignalStatusList_gen, Gitsn_SignalStatusList_del, Gitsn_SignalStatusList_append, Gitsn_SignalStatusList_cnt, Gitsn_SignalStatusList_tlv, Dz1Text("Gitsn_SignalStatus") )).code) ERR_OUT(errp);
...
Dz1JsonSpec *GitsnDSRCMsgTestScenarioDef_genJsonSpec(Dz1Error *err)
{
	DZ1_ERROR_SAFE_PTR(errp, err);
	...
	else
	{
		...
		if (GitsnDSRCMsgTestScenarioDef_initJsonSpec(ret, errp) == FALSE) ERR_OUT(errp);
