...
#ifdef WIN32
	if ((sh = Dz1Shell_new(Dz1Text("win32crt"), DZ1_SHELL_CMD_ALL, 16, Dz1Text("root"), Dz1Text("GitsnDSRCMsgTestScenario"), errp)) == NULL) ERR_OUT(errp);
#else
	if ((sh = Dz1Shell_new(Dz1Text("xterm"), DZ1_SHELL_CMD_ALL, 16, Dz1Text("root"), Dz1Text("GitsnDSRCMsgTestScenario"), errp)) == NULL) ERR_OUT(errp);
#endif
	else
	{
...
{
	Dz1Thread_log(DZ1_THREAD_LOG_ERROR, "GitsnDSRCMsgTestScenario : Exception Catch\n");
	ERR_OUT(reason);
}
...
static bool_t _process_start(bool_t is_daemon, Dz1Error *errp)
{
	...
	if ((logfp = Dz1FileStream_open(Dz1Text("GitsnDSRCMsgTestScenarioTest.log.txt"), Dz1Text("wb"), NULL)) != NULL)
	{
	...
	else
	{
		...
		if ((env.task = GitsnDSRCMsgTestScenario_new(NULL, NULL, _task_exception_handler, NULL, errp)) == NULL) ERR_OUT(errp);	// Edit Task Argument
