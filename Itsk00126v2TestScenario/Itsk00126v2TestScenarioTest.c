...
static bool_t _process_start(bool_t is_daemon, Dz1Error *errp)
{
	...
	if ((logfp = Dz1FileStream_open(Dz1Text("Itsk00126v2TestScenarioTest.log.txt"), Dz1Text("wb"), NULL)) != NULL)
	{
	...
	else
	{
		...
		if ((env.task = Itsk00126V2TestScenario_new(NULL, NULL, _task_exception_handler, NULL, errp)) == NULL) ERR_OUT(errp);	// Edit Task Argument
