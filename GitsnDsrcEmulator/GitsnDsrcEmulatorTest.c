...
static void cmd_init_usage(Dz1Str n) { Dz1Thread_printf(Dz1Text("Usage] %s port\n"), n); }
static Dz1ShellCmdReturn cmd_init(void* ptr, int argc, Dz1Str argv[])
{
	...
	else
	{
		...
		else if (GitsnDsrcEmulatorTest_init(env->task, port, errp) == FALSE) ERR_OUT(errp);
		else _print_usage(n);
	}
...
////////////////////////////////////////////////////////////////////////////////
// cmd_tdpsend Root Command
static void cmd_tdpsend_usage(Dz1Str n) { Dz1Thread_printf(Dz1Text("Usage] %s id\n"), n); }
static Dz1ShellCmdReturn cmd_tdpsend(void* ptr, int argc, Dz1Str argv[])
{
	DZ1_ERROR_SAFE_VAR(errp, err);
	void (*_print_usage)(Dz1Str n) = cmd_tdpsend_usage;
	Dz1Str n = argv[0];
	TestEnv *env = (TestEnv *)ptr;
	if (argc != 2) _print_usage(n);
	else
	{
		...
		if (errp->code) {}
		//		else if (Dz1STRCMP(argv[1], Dz1Text("_long_sub_cmd_")) == 0 || Dz1STRCMP(argv[1], Dz1Text("_short_sub_cmd_")) == 0) _sub_cmd_(ptr, n, argc-1, &argv[1]);
		else if (GitsnDsrcEmulatorTest_tdpsend(env->task, id, errp) == FALSE) ERR_OUT(errp);
		else _print_usage(n);
	}
	return Dz1ShellCmdReturn_ok;
}
...
#ifdef WIN32
	if ((sh = Dz1Shell_new(Dz1Text("win32crt"), DZ1_SHELL_CMD_ALL, 16, Dz1Text("root"), Dz1Text("GitsnDsrcEmulator"), errp)) == NULL) ERR_OUT(errp);
#else
	if ((sh = Dz1Shell_new(Dz1Text("xterm"), DZ1_SHELL_CMD_ALL, 16, Dz1Text("root"), Dz1Text("GitsnDsrcEmulator"), errp)) == NULL) ERR_OUT(errp);
#endif
	else
	{
		...
		else if ((*errp = Dz1Shell_cmdReg(sh, Dz1Text("init"), cmd_init, task, Dz1Text("connect to server"))).code) ERR_OUT(errp);
		else if ((*errp = Dz1Shell_cmdReg(sh, Dz1Text("tdpsend"), cmd_tdpsend, task, Dz1Text("tdpsend") )).code) ERR_OUT(errp);
		else
		{
...
{
	Dz1Thread_log(DZ1_THREAD_LOG_ERROR, "GitsnDsrcEmulator : Exception Catch\n");
	ERR_OUT(reason);
}
...
static bool_t _process_start(bool_t is_daemon, Dz1Error *errp)
{
	...
	if ((logfp = Dz1FileStream_open(Dz1Text("GitsnDsrcEmulatorTest.log.txt"), Dz1Text("wb"), NULL)) != NULL)
	{
	...
	else
	{
		...
		else
		{
			...
			if ((env.task = GitsnDsrcEmulator_new(_task_exception_handler, NULL, errp)) == NULL) ERR_OUT(errp);	// Edit Task Argument
