...
static int cp1258_mbtowc(conv_t conv, ucs4_t *pwc, const unsigned char *s, int n)
{
	unsigned char c = *s;
	unsigned short wc;
	...
	if (last_wc)
	{
		if (wc >= 0x0300 && wc < 0x0340)
		{
			...
			if (last_wc >= viet_comp_table_data[i1].base && last_wc <= viet_comp_table_data[i2].base)
			{
				...
				{
					i = (i1+i2)>>1;
					if (last_wc == viet_comp_table_data[i].base) break;
					if (last_wc < viet_comp_table_data[i].base)
					{
						if (i1 == i) goto not_combining;
						i2 = i;
					}
					else
					{
...
static int cp1258_wctomb(conv_t conv, unsigned char *r, ucs4_t wc, int n)
{
	unsigned char c = 0;
	if (wc < 0x0080)
	{
	...
	/* Try canonical decomposition. */
	{
		...
		if (wc >= viet_decomp_table[i1].composed && wc <= viet_decomp_table[i2].composed)
		{
			...
			for (;;)
			{
				/* Here i2 - i1 > 0. */
				i = (i1+i2)>>1;
				if (wc == viet_decomp_table[i].composed) break;
				if (wc < viet_decomp_table[i].composed)
				{
					if (i1 == i) return RET_ILUNI;
					/* Here i1 < i < i2. */
					i2 = i;
				}
				else
