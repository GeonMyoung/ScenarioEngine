...
static int cp1255_mbtowc(conv_t conv, ucs4_t *pwc, const unsigned char *s, int n)
{
	unsigned char c = *s;
	unsigned short wc;
			...
			if (last_wc >= cp1255_comp_table_data[i1].base && last_wc <= cp1255_comp_table_data[i2].base)
			{
				...
				for (;;)
				{
					...
					else
					{
						...
						{
							i = i2;
							if (last_wc == cp1255_comp_table_data[i].base) break;
							goto not_combining;
						}
					}
				}
...
static int cp1255_wctomb(conv_t conv, unsigned char *r, ucs4_t wc, int n)
{
	unsigned char c = 0;
	if (wc < 0x0080)
	{
	...
	/* Try canonical decomposition. */
	{
		...
		if (wc >= cp1255_decomp_table[i1].composed && wc <= cp1255_decomp_table[i2].composed)
		{
			...
			for (;;)
			{
				/* Here i2 - i1 > 0. */
				i = (i1+i2)>>1;
				if (wc == cp1255_decomp_table[i].composed) break;
