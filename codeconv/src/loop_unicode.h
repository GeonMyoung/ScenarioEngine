    ...
    if (ret != RET_ILUNI) {
      ...
      for (i = 0; i < ret; i++) {
        ...
        if (sub_outcount <= RET_ILUNI)
          goto johab_hangul_failed;
        if (!((size_t)sub_outcount <= outleft)) abort();
...
static size_t unicode_loop_convert (codeconv_t icd,
...
outcount_ok:
			...
			if (!((size_t)outcount <= outleft)) abort();
...
static size_t unicode_loop_reset (codeconv_t icd,
                                  char* * outbuf, size_t *outbytesleft)
{
  conv_t cd = (conv_t) icd;
